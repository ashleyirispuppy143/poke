...
* **Vibecoding:** The practice of generating software logic entirely through natural language prompts and submitting it without manually reading, comprehending, or verifying the underlying code. It relies on the assumption that if the output *appears* to work (passes a casual "vibe check"), it is safe to merge.
...
> **In short:** You can use AI for up to 75% of your code in most cases, or 90% for specific routine tasks. Generative AI assets, 100% AI documentation, vibecoding, and using AI for security logic without extreme oversight are banned.
...
* **Vibecoding:** You **MUST NOT** engage in "vibecoding". Submitting AI-generated code that you do not personally understand, cannot explain, or have not thoroughly tested is strictly prohibited. You are accountable for the logic, not just the prompt.
...
We recognize that code origins can sometimes be ambiguous, and AI detection tools are deeply flawed. To protect contributors and maintain a welcoming environment, we operate on a principle of trust and verification.

**Code from External Platforms (e.g., StackOverflow, Forums):**
When integrating snippets from external free software repositories, wikis, or forums, the origin of the code may be unknown. If you are genuinely unsure whether the original author utilized generative AI, you do not need to apply an AI disclosure trailer. However, you **MUST** thoroughly review and test the logic to prevent accidental "vibecoding" by proxy. You remain entirely accountable for the code's quality, security, and license compliance, and must follow standard free software attribution practices (e.g., linking the original thread).

**Legacy Code and Older Projects:**
Copying boilerplate, logic, or scripts from older, pre-LLM codebases does not require disclosure, even if a modern AI model would generate something remarkably similar today. Highly optimized code often looks identical to generated patterns.

**The "False Positive" Rule:**
The Poke Project strictly prohibits relying on automated "AI detection" tools. These tools are notoriously unreliable and frequently flag neurodivergent phrasing, non-native English writing, or highly structured coding styles as AI-generated.
...
1.  **Rejected Contributions:** PRs or MRs found to violate the 75%/90% code caps (without prior exception), or containing prohibited Generative AI Assets, vibecoded logic, automated documentation, or security flaws, **WILL** be immediately closed. 
...
* Clarified the difference between rejecting code for lack of comprehension vs. rejecting code based on subjective "AI vibes."
* *(Note: Versions 2.1.3-final and 2.1.4-final are preserved in the changelog as previous final draft iterations.)*
