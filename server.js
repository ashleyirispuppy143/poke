...
const logPrefix = ENABLE_CLUSTER ? (cluster.isPrimary ? "[Primary]" : `[Worker ${cluster.worker.id}]`) : "[Server]";
...
['log', 'info'].forEach(method => {
    ...
    console[method] = (...args) => {
        if (ENABLE_CLUSTER && cluster.isWorker &&
            cluster.worker.id !== 1 && isBooting) return;
...
function getGlobalCpu() {
    ...
    for (let i = 0; i < cpus.length; i++) {
        ...
        idle += times.idle;
        total += times.user + times.nice + times.sys + times.irq +
            times.idle;
    }
    return {
        idle,
...
if (ENABLE_CLUSTER && cluster.isPrimary) {
    ...
    cluster.on('message', (worker, msg) => {
        ...
        // Seamless Handoff: Master handles spawning the replacement instantly
        if (msg && msg.type === 'spawn_replacement') {
            if (worker.hasSpawnedReplacement) return; // Strict ghost/dupe prevention
            worker.hasSpawnedReplacement = true;
            console.warn(`[Primary] Worker ${worker.process.pid} thread maxed out. Spawning silent replacement...`);
            cluster.fork();
        }
    });
    ...
    cluster.on("disconnect", (worker) => {
        ...
        // We no longer fork here for load shedding, 'spawn_replacement' IPC handles it to prevent duplicates
    });

    cluster.on("exit", (worker, code, signal) => {
        delete clusterStats.workers[worker.id];
        if (!worker.exitedAfterDisconnect && !worker.hasSpawnedReplacement) {
    ...
    (async function() {
        ...
        const media_proxy =
            require("./src/libpoketube/libpoketube-video.js");
        const {
            sinit
        } =
        require("./src/libpoketube/init/superinit.js");
        const innertube =
            require("./src/libpoketube/libpoketube-youtubei-objects.json");
        const fs =
            require("fs");
        ...
        (function GlobalServerOptimizer() {
            ...
            // HTOP-Style Accurate Single Thread CPU Monitor
            ...
            setInterval(() => {
                ...
                const userMicros = currentUsage.user - lastCpuUsage.user;
                const sysMicros = currentUsage.system - lastCpuUsage.system;
                const totalMicros = userMicros + sysMicros;

                // elapsed time in microseconds
                const elapsedMicros = Number(currentHrTime - lastHrTime) / 1000;

                // htop style: 100% means 1 full logical core is saturated by this specific worker
                const threadCpuPercent = (totalMicros / elapsedMicros) * 100;

                if (threadCpuPercent >= 95) { // Protect against thread locking
                    cpuStrikes++;
                    const strikeLimit = 2; // Trigger quickly if pinned
                    if (cpuStrikes >= strikeLimit) {
                        console.warn(`[CPU SENTINEL] Worker ${process.pid} thread saturated at ${threadCpuPercent.toFixed(1)}%. Initiating 0-downtime silent replacement.`);
                        isDisconnecting = true;

                        // 1. Tell primary to spawn replacement immediately for smooth handoff
                        process.send({
                            type: 'spawn_replacement'
                        });

                        // 2. Disconnect to route new traffic to the fresh worker, while finishing active requests
                        cluster.worker.disconnect();

                        // 3. Absolute cleanup after 2 seconds (prevents hanging / ghosts)
                        setTimeout(() => {
                            console.error(`[CPU SENTINEL] Worker ${process.pid} completely cleaned up after 2s.`);
                            process.exit(1);
                        }, 2000).unref();
                    }
                } else {
        ...
        initlog(`[Welcome] Welcome To Poke, where you can LIBERATE THE WEB! - :3  +         
Running  +      Node ${process.version} - V8 v${            process.versions.v8          } -  
${process.platform.replace("linux", "GNU/Linux")} ${            process.arch    
        ...
        const {
            ieBlockMiddleware
        } = require("./src/libpoketube/ptutils/ie-blocker.js");
        ...
        const CLOUDFLARE_V4 = ["173.245.48.0/20", "103.21.244.0/22",
        ...
        (function configureTrustProxy() {
            ...
            function detectEnvSignals() {
                ...
                for (const [key, name] of Object.entries(signals)) {
                    if (process.env[key]) detected.push({
                        key,
                        name,
            ...
            function detectFilesystemSignals() {
                ...
                try {
                    ...
                    for (const [re, name] of patterns) {
                        if (re.test(cgroup)) signals.push({
                            path: "/proc/1/cgroup",
                            name
                        });
                    }
            ...
            function logProxyHeaders(req) {
                const interesting = [
                    ...
                    "x-real-ip", "via", "cf-ray", "cf-connecting-ip", "fly-request-id", "x-amzn-trace-id",
                ];
        ...
        (function PokeResourceGuard() {
            ...
            const eldHistogram = monitorEventLoopDelay({
                resolution: 20
            });
            eldHistogram.enable();
            ...
            const EXACT_STATUS_ROUTES = new Set(["/robots.txt", "/favicon.ico",
                "/_pokeresource/stats", "/_poketraffic/stats", "/_pokestopskids/stats",
                "/_pokeoverload/stats", "/health", "/traffic"
            ]);
            ...
            function getTopMapEntries(map, limit) {
                return Array.from(map.entries()).sort((a, b) => b[1] - a[1]).slice(0, limit).map(entry => ({
                    key: entry[0],
                    count: entry[1]
                }));
            }
            ...
            function isKnownBot(ua) {
                return !ua ? false : BOT_MEGA_REGEX.test(ua);
            }
            ...
            function sampleCpuAndMemory(reason) {
                ...
                const idleDiff = currentSampleCpu.idle - lastSampleCpu.idle;
                const totalDiff = currentSampleCpu.total - lastSampleCpu.total;
                const cpuPercent = totalDiff === 0 ? 0 : 100 - (100 * idleDiff / totalDiff);
                const cpuRatio = cpuPercent / 100;
            ...
            function classifyResourcePressure(snapshot) {
                ...
                addPressure("Global_OS_CPU", snapshot.cpu.ratio, cfg.warmCpuRatio, cfg.stressedCpuRatio, cfg.criticalCpuRatio, "");
                ...
                if (hasCritical || score >= 7) consecutiveCriticalSpikes++;
                else consecutiveCriticalSpikes = Math.max(0, consecutiveCriticalSpikes - 1);

                if (consecutiveCriticalSpikes >= 5) return {
                    state: "critical",
                    score,
                    reasons
                };
                if (score >= 4 || consecutiveCriticalSpikes > 0) return {
                    state: "stressed",
                    score,
                    reasons
                };
            ...
            function getAdmissionDecision(req, client, kind, now) {
                ...
                if (client.cooldownUntil > now) return {
                    action: "reject",
                ...
                if (state === "warm") {
                    if ((kind === "heavy" || kind === "background") && noisy && getMetric(client, "heavy", now) > getHeavyLimit(state)) return {
                        action: "reject",
                ...
                if (state === "stressed") return {
                    action: "allow",
                    reason: "stressed-pass-as-normal"
                };

                if (state === "critical") {
                    if (kind === "page") {
                        if (!noisy && getMetric(client, "softPass", now) < resourceConfig.client.pageSoftPassesPerWindow) return {
            ...
            function sendResourceStats(req, res) {
                if (!isGuardActive) return res.status(503).json({
                    message: "Guard is initializing, please wait..."
                });
                res.json(getResourceStats());
            }

            const HEALTH_CSS = `@font-face { font-family: "PokeTube Flex"; src: url("/static/robotoflex.ttf"); font-style: normal; font-stretch: 1% 800%; font-display: swap; } :root{color-scheme:dark} body{color:#fff;background:#1c1b22;margin:0;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;} a{color:#0ab7f0;text-decoration:none;transition:color 0.2s;} a:hover{color:#00c0ff;text-decoration:underline;} .app{max-width:1100px;margin:0 auto;padding:24px;} h1,h2,h3,.tab-btn,.hero-label,.route-rank{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;} h1,h2{font-stretch:extra-expanded;letter-spacing:-0.5px;} h1{font-weight:900;font-size:2.2rem;margin-top:0;margin-bottom:4px;} h2{margin-top:32px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:8px;} p,li,code,pre{line-height:1.6;} hr{border:0;border-top:1px solid rgba(255,255,255,0.1);margin:32px 0;} .logo{float:right;margin:.3em 0 1em 2em;max-width:130px;} .header-container{display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;margin-bottom:24px;gap:16px;} .tabs{display:inline-flex;background:#15141a;border-radius:24px;padding:4px;border:1px solid rgba(255,255,255,0.05);flex-wrap:wrap;gap:2px;} .tab-btn{background:transparent;color:#aaa;border:none;padding:8px 20px;border-radius:20px;cursor:pointer;font-weight:700;font-size:0.95rem;transition:all 0.3s ease;outline:none;display:inline-block;line-height:1.2;} .tab-btn:hover:not(.active){color:#fff;text-decoration:none;} .tab-btn.active{background:#0ab7f0;color:#1c1b22;box-shadow:0 2px 8px rgba(10,183,240,0.3);} .hero-stat{padding:16px 0 32px 0;text-align:center;} .hero-label{font-size:1.1rem;color:#aaa;margin-top:8px;text-transform:uppercase;letter-spacing:2px;} .hero-num{font-size:4rem;font-weight:900;color:#0ab7f0;margin:10px 0;line-height:1;} .privacy-badge{display:inline-flex;align-items:center;color:#4caf50;padding:6px 12px;border-radius:10px;font-size:0.9rem;font-weight:bold;margin-bottom:16px;} .stat-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:16px;margin-top:16px;} .stat-box{background:#2a2930;border-radius:12px;padding:16px;margin:0;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;border:1px solid rgba(255,255,255,0.05);} .stat-num{font-size:1.6rem;color:#0ab7f0;display:block;font-weight:bold;text-transform:capitalize;} .stat-label{font-size:.9rem;color:#aaa;display:block;margin-top:4px;} .green{color:#4caf50} .orange{color:#ff9800} .red{color:#f44336} code,pre{background:#2a2930;padding:2px 6px;border-radius:4px;} pre{overflow:auto;padding:14px;border:1px solid #333;} .banner{padding:16px 20px;border-radius:12px;background:#2a2930;border:1px solid rgba(255,255,255,0.05);transition:border-left 0.3s ease;} .banner.green{border-left:5px solid #4caf50} .banner.orange{border-left:5px solid #ff9800} .banner.red{border-left:5px solid #f44336} .small{color:#bbb;font-size:.95rem;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;} .explanation{font-size:1.05rem;color:#ddd;background:#2a2930;padding:20px;border-radius:12px;border-left:4px solid #0ab7f0;margin-bottom:24px;} summary{font-size:1.2rem;cursor:pointer;color:#0ab7f0;user-select:none;margin-bottom:12px;font-weight:bold;transition:color 0.2s;font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;} .route-list{display:flex;flex-direction:column;gap:12px;margin-top:16px;} .route-item{background:#25242b;border-radius:12px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;border:1px solid rgba(255,255,255,0.05);position:relative;overflow:hidden;} .route-bar{position:absolute;left:0;top:0;bottom:0;background:linear-gradient(90deg,rgba(10,183,240,0.05) 0%,rgba(10,183,240,0.15) 100%);z-index:1;transition:width 0.6s cubic-bezier(0.22,1,0.36,1);} .route-info{display:flex;align-items:center;gap:12px;z-index:2;} .route-rank{font-size:0.85rem;font-weight:900;color:#1c1b22;background:#0ab7f0;width:28px;height:28px;border-radius:50%;display:flex;justify-content:center;align-items:center;} .route-name{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;font-size:1.05rem;color:#eee;padding:4px 8px;border-radius:6px;background:rgba(255,255,255,0.05);letter-spacing:-0.2px;} .route-count{font-size:1.15rem;font-weight:900;color:#fff;z-index:2;background:rgba(0,0,0,0.3);padding:6px 14px;border-radius:20px;border:1px solid rgba(255,255,255,0.1);} @media (max-width:640px){.hero-num{font-size:3rem} .route-item{align-items:flex-start;gap:12px;flex-direction:column} .route-count{align-self:flex-end}}`;

            function formatRouteCount(c) {
                if (c === 67) return '67 (really)';
                if (c === 69) return '69 (haha nice)';
                if (c === 420) return '420 (some weed everyday!)';
            ...
            function sendAntiddosPage(req, res) {
                ...
                cachedHealthHtml = `<!DOCTYPE html><html><head><style>${HEALTH_CSS}</style></head><body><h1>Poke Server Health</h1><noscript>JavaScript is disabled: this is a static little snapshot of Poke's server mood. Refresh the page whenever you want fresh numbers.</noscript><h2>Live server Health!!!</h2><h3>Server Vitals</h3><h3>Requests</h3><h3>Anonymous Stats</h3><p>Current Status: ${stats.state.state}</p><p>Load Score: ${stats.state.score} • Process Delay: ${stats.state.eventLoop.p99Ms}ms • CPU: ${stats.state.cpu.percent}% • Memory: ${formatPercent(stats.state.memory.rssRatio)}</p><h2>What is this page?</h2><p>This is Poke checking in on itself so everything stays fast and comfy. It watches CPU, memory, request pressure, and process delay in real time, then uses that info to keep the site feeling smooth. If traffic suddenly gets wild, Poke gently shields the server and tries to keep normal video watching flowing first. Basically, this page is the little heartbeat monitor for the whole instance!!</p></body></html>`;
...
EL p99 warm/stressed/critical:  + ${resourceConfig.system.eventLoop.warmMs} / ${resourceConfig.system.eventLoop.stressedMs} / ${resourceConfig.system.eventLoop.criticalMs}`
        ...
        initlog("Loaded EJS");
        app.use(modules.express.urlencoded({
            extended: true
        }));
