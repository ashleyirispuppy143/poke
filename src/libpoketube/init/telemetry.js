...
const cleanPageTitleCache = new Map()
function cleanPageTitle(value) {
  const str = String(value || "")
  if (!str) return ""
  if (cleanPageTitleCache.has(str)) return cleanPageTitleCache.get(str)

  let title = str.replace(/\s+/g, " ").trim()
  if (!title) {
    cleanPageTitleCache.set(str, "")
    return ""
  }
...
  if (/^Poke(?:Tube)?$/i.test(title)) title = ""
  else title = title.slice(0, 300)

  if (cleanPageTitleCache.size > 10000) cleanPageTitleCache.clear()
  cleanPageTitleCache.set(str, title)
  return title
}
...
    const valid = []
    for (let i = input.recentVideos.length - 1; i >= 0; i--) {
      const id = String(input.recentVideos[i] || "").trim()
      if (id && !unique.has(id)) {
        unique.add(id)
        valid.unshift(id)
      }
    }
    stats.recentVideos = valid.slice(-SETTINGS.telemetry.recentLimit)
  }
...
    const merged = [...(target.recentVideos || []), ...clean.recentVideos]
    const unique = new Set()
    const valid = []
...
      }
    }
    target.recentVideos = valid.slice(-SETTINGS.telemetry.recentLimit)
  }
...
      // Remove whitespace format to massively drop CPU blockage
...
  const users = {}
  for (const key of userKeys.slice(-SETTINGS.telemetry.usersLimit)) {
    users[key] = true
  }
...
    if (!result.ok) console.error("Could not read telemetry file, starting with empty telemetry:", SETTINGS.telemetry.storage.path, result.error)
    const empty = getEmptyStats()
    lastJsonByFile.set(SETTINGS.telemetry.storage.path, JSON.stringify(empty))
    return empty
  }

  const clean = normalizeStats(result.data)
  lastJsonByFile.set(SETTINGS.telemetry.storage.path, JSON.stringify(clean))
  return clean
}

const YOUTUBE_CATEGORY_ID_TO_GENRE = Object.freeze({
  "1": "film_animation", "2": "autos_vehicles", "10": "music", "15": "pets_animals",
  "17": "sports", "19": "travel_events", "20": "gaming", "22": "people_blogs",
  "23": "comedy", "24": "entertainment", "25": "news_politics", "26": "howto_style",
  "27": "education", "28": "science_technology", "29": "nonprofits_activism",
  "30": "film_animation", "31": "film_animation", "32": "film_animation",
  "33": "film_animation", "34": "comedy", "35": "education", "36": "film_animation",
  "37": "entertainment", "38": "film_animation", "39": "film_animation",
  "40": "film_animation", "41": "film_animation", "42": "shorts", "43": "shows", "44": "trailers"
})

const GENRE_ALIASES = Object.freeze({
  "autos": "autos_vehicles", "autos & vehicles": "autos_vehicles", "auto": "autos_vehicles",
  "cars": "autos_vehicles", "vehicles": "autos_vehicles", "film": "film_animation",
  "film & animation": "film_animation", "animation": "film_animation", "anime": "film_animation",
  "movies": "film_animation", "movie": "film_animation", "music": "music", "song": "music",
  "pets": "pets_animals", "animals": "pets_animals", "pets & animals": "pets_animals",
  "sports": "sports", "sport": "sports", "travel": "travel_events",
  "travel & events": "travel_events", "events": "travel_events", "gaming": "gaming",
  "games": "gaming", "game": "gaming", "people": "people_blogs", "people & blogs": "people_blogs",
  "blogs": "people_blogs", "vlog": "people_blogs", "vlogs": "people_blogs", "comedy": "comedy",
  "funny": "comedy", "entertainment": "entertainment", "news": "news_politics",
  "news & politics": "news_politics", "politics": "news_politics", "howto": "howto_style",
  "how-to": "howto_style", "howto & style": "howto_style", "how to & style": "howto_style",
  "style": "howto_style", "fashion": "howto_style", "beauty": "howto_style",
  "education": "education", "educational": "education", "documentary": "education",
  "science": "science_technology", "technology": "science_technology",
  "science & technology": "science_technology", "tech": "science_technology",
  "nonprofits": "nonprofits_activism", "activism": "nonprofits_activism",
  "nonprofits & activism": "nonprofits_activism", "shorts": "shorts", "short": "shorts",
  "shows": "shows", "show": "shows", "trailers": "trailers", "trailer": "trailers"
})
...
      },
      regex: [["\\b(?:ft\\.|feat\\.|featuring)\\b", 4], ["\\b\\d+d audio\\b", 4]]
    },
    gaming: {
      phrases: [
...
        speedrun: 9, walkthrough: 9, mod: 5, mods: 5, dlc: 5, fps: 5, rpg: 5, simulator: 4
      },
      regex: [["\\b(?:ps5|ps4|xbox|switch|pc)\\b", 3], ["\\b(?:episode|part)\\s+\\d+\\b", 2]]
    },
    news_politics: {
      phrases: [
...
      },
      regex: [["\\b(?:cnn|bbc|reuters|ap|al jazeera|sky news|fox news|nbc news|abc news|cbs news)\\b", 7]]
    },
    sports: {
      phrases: [
...
      },
      regex: [["\\b(?:rtx|gtx|ryzen|intel|amd|nvidia|apple m\\d+)\\b", 6]]
    },
    education: {
      phrases: [
...
        mathematics: 7, guide: 5, study: 5, classroom: 6, facts: 4, analysis: 4
      },
      regex: [["\\b(?:101|beginners?|advanced)\\b", 3]]
    },
    comedy: {
      phrases: [
...
      ],
      tokens: { comedy: 10, funny: 8, meme: 7, memes: 7, skit: 8, parody: 8, prank: 6, jokes: 7, comedian: 9, laugh: 6, bloopers: 6 },
      regex: []
    },
    film_animation: {
      phrases: [
...
      ],
      tokens: { movie: 7, film: 7, trailer: 8, teaser: 8, anime: 7, animation: 8, animated: 7, cinema: 6, scene: 5, clip: 4, episode: 4 },
      regex: []
    },
    howto_style: {
      phrases: [
...
      ],
      tokens: { makeup: 8, beauty: 8, skincare: 8, fashion: 8, outfit: 7, haul: 6, grwm: 9, hair: 6, nails: 6, style: 5, styling: 6, diy: 6, recipe: 6, cooking: 6, cook: 5, baking: 6, makeover: 6 },
      regex: []
    },
    travel_events: {
      phrases: [
...
      ],
      tokens: { travel: 8, vlog: 4, trip: 7, airport: 6, flight: 6, hotel: 6, beach: 5, island: 5, city: 3, tour: 5, cruise: 6, vacation: 6 },
      regex: []
    },
    autos_vehicles: {
      phrases: [
...
      ],
      tokens: { car: 7, cars: 7, vehicle: 7, vehicles: 7, auto: 6, automotive: 8, truck: 7, suv: 7, motorcycle: 8, bike: 4, tesla: 7, engine: 6, horsepower: 7, drifting: 7 },
      regex: [["\\b(?:bmw|mercedes|toyota|honda|ford|tesla|audi|porsche|ferrari|lamborghini)\\b", 5]]
    },
    pets_animals: {
      phrases: [["funny cats", 9], ["funny dogs", 9], ["pet care", 8], ["animal rescue", 9]],
      tokens: { cat: 7, cats: 7, dog: 7, dogs: 7, pet: 7, pets: 7, animal: 7, animals: 7, kitten: 8, puppy: 8, wildlife: 7, zoo: 6, rescue: 5 },
      regex: []
    },
    nonprofits_activism: {
      phrases: [["climate change", 8], ["human rights", 8], ["fundraiser", 8], ["mutual aid", 8], ["nonprofit", 9], ["charity stream", 9]],
      tokens: { nonprofit: 9, charity: 8, activism: 8, activist: 8, protest: 7, rights: 5, fundraiser: 8, donation: 7, climate: 6, environment: 6 },
      regex: []
    },
    people_blogs: {
      phrases: [["day in my life", 9], ["life update", 8], ["story time", 8], ["storytime", 8], ["daily vlog", 9]],
      tokens: { vlog: 7, vlogs: 7, family: 5, life: 4, routine: 4, qna: 5, qanda: 5, personal: 5 },
      regex: [["\\bq\\s*&\\s*a\\b", 5]]
    },
    shorts: {
...
    },
    entertainment: {
      phrases: [["reaction video", 6], ["behind the scenes", 5], ["celebrity interview", 7]],
      tokens: { entertainment: 8, reaction: 5, celebrity: 6, drama: 4, challenge: 4, viral: 4, tiktok: 4 },
      regex: []
    }
  }
})
...
const hourMsCache = new Map()
function getHourMsFromKey(key) {
  if (hourMsCache.has(key)) return hourMsCache.get(key)
  const ms = Date.parse(`${key}:00:00.000Z`)
  const result = Number.isFinite(ms) ? ms : 0
  if (hourMsCache.size > 5000) hourMsCache.clear()
  hourMsCache.set(key, result)
  return result
}

const genreNameCache = new Map()
function normaliseGenreName(value) {
  const str = String(value || "")
  if (genreNameCache.has(str)) return genreNameCache.get(str)
  
  const raw = str.replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim().toLowerCase()
  const result = raw ? (GENRE_ALIASES[raw] || raw.replace(/\s+/g, "_")) : ""
  
  if (genreNameCache.size > 2000) genreNameCache.clear()
  genreNameCache.set(str, result)
  return result
}
...
      for (let i = 0; i < value.length; i++) add(value[i])
      return
    }

    if (value && typeof value === "object") {
      const vals = Object.values(value)
      for (let i = 0; i < vals.length; i++) add(vals[i])
      return
    }
...
  for (let i = 0; i < values.length; i++) add(values[i])
  return out
}
...
        return { pattern: new RegExp(item[0], "i"), weight: Number(item[1]) || 0 }
      }
      return { pattern: new RegExp(String(item || ""), "i"), weight: 5 }
    })
  }))
...
const genreInferenceCache = new Map()

function inferTrendingGenreFromTitle(value) {
  const title = cleanPageTitle(value)
  if (!title || isMissingRecordedTitle(title)) {
    return { category: TRENDING_GENRE_MODEL.lowConfidence, confidence: 0, source: "missing_title" }
  }

  const cacheKey = String(value || "").trim()
  if (genreInferenceCache.has(cacheKey)) return genreInferenceCache.get(cacheKey)
...
    if (score > 0) results.push({ category: model.category, score, matches })
  }
...
  if (!best) {
    finalResult = { category: TRENDING_GENRE_MODEL.lowConfidence, confidence: 0.15, source: "title_heuristic" }
  } else {
...
  if (genreInferenceCache.size > 10000) genreInferenceCache.clear()
  genreInferenceCache.set(cacheKey, finalResult)
  return finalResult
}
...
  if (categoryIdGenre) {
    return { category: categoryIdGenre, confidence: 1, source: "youtube_category_id" }
  }

  const suppliedGenre = getFirstSupportedGenreFromValues([
    options.category, options.genre, options.videoGenre
  ])

  if (suppliedGenre) {
    return { category: suppliedGenre, confidence: 0.98, source: "supplied_category" }
  }

  const suppliedTagGenre = getFirstSupportedGenreFromValues([
    options.tags, options.genreTags, options.videoGenreTags, options.videoTags
  ])

  if (suppliedTagGenre) {
    return { category: suppliedTagGenre, confidence: 0.93, source: "supplied_genre_tags" }
  }
...
  if (tagText) {
    const inferred = inferTrendingGenreFromTitle(`${title || ""} ${tagText}`.trim())
...
  if (!result.ok || !result.data) {
    if (!result.ok) console.error("Could not read trending file, starting with empty trending:", SETTINGS.trending.storage.path, result.error)
    return getEmptyTrending()
  }

  const clean = normalizeTrending(result.data)
  lastJsonByFile.set(SETTINGS.trending.storage.path, JSON.stringify(clean))
  return clean
}
...
    if (shouldReplaceRecordedTitle(existing.title, cleanTitle)) existing.title = cleanTitle
    if (metadata.categoryId || metadata.videoCategoryId) existing.categoryId = metadata.categoryId || metadata.videoCategoryId
    if (metadata.genre || metadata.videoGenre || metadata.category) existing.genre = metadata.genre || metadata.videoGenre || metadata.category
...
    if (requestedCategory && requestedCategory !== "all" && requestedCategory !== category) continue
...
  if (uaCache.size > 5000) uaCache.clear()
...
  if (!entries.length) return '<div class="breakdown-empty">No data recorded yet.</div>'
...
  const sortedVideos = Object.entries((stats && stats.videos) || {}).sort((a, b) => b[1] - a[1])
...
  const totalDetections = Math.max(sumObjectValues((stats && stats.os) || {}), sumObjectValues((stats && stats.browsers) || {}))
...
  const views = typeof options.views === "undefined" ? "" : `<div class="video-views">${escapeHtml(formatServerLocalViewCount(options.views))}</div>`
...
  const selectedPanel = tab === "recent" ? recentPanel : tab === "trending" ? trendingPanel : tab === "topvideos" ? topPanel : tab === "api" ? apiPanel : overviewPanel
...
    :visited{color:#00c0ff} a{color:#0ab7f0} button{font:inherit} .app{max-width:1100px;margin:0 auto;padding:24px}
...
    h1{font-weight:1000;font-stretch:ultra-expanded;margin-top:0;margin-bottom:.35rem} h2{font-weight:700;font-stretch:extra-expanded;margin-top:0;margin-bottom:.4rem}
    h3{font-weight:700;font-stretch:extra-expanded;margin:0 0 .75rem 0;font-size:1.02rem} .note{color:#bbb;font-size:.95rem} .small{color:#bbb;font-size:.95rem}
    .logo{float:right;margin:.3em 0 1em 2em;max-width:130px} .header-container{display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;margin-bottom:24px;gap:16px}
...
    .tab-btn:hover:not(.active){color:#fff;text-decoration:none} .tab-btn.active{background:#0ab7f0;color:#1c1b22;box-shadow:0 2px 8px rgba(10,183,240,.3)}
...
    .telemetry-notice h2{margin:0 0 .4rem 0;font-size:1.05rem} .telemetry-notice p{margin:.4rem 0 0 0}
    .hero{display:grid;grid-template-columns:1.45fr .95fr;gap:16px;align-items:start;margin-bottom:18px} .hero-main p,.hero-side p{margin:.4rem 0 0 0}
    .hero-side{display:flex;flex-direction:column;gap:14px} .mini-stat{display:flex;flex-direction:column;gap:.15rem}
    .mini-stat-label{color:#bbb;font-size:.92rem} .mini-stat-value{font-size:1.55rem;font-weight:700;display:flex;align-items:center;gap:.45rem;flex-wrap:wrap}
    .mini-stat-sub{color:#bbb;font-size:.9rem;line-height:1.45} .segmented{display:flex;gap:10px;flex-wrap:wrap;margin:0 0 18px 0}
...
    .seg-btn.active{border-color:#0ab7f0;box-shadow:inset 0 0 0 1px #0ab7f0;background:#1f1e29} .section-card{margin-bottom:16px}
    .section-help{margin-bottom:16px;background:#22212d;border:1px solid #2e2d3b;border-radius:16px;padding:16px} .section-help p{margin:.35rem 0 0 0}
    .overview-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px} .breakdown-empty{color:#bbb} .breakdown-list{display:flex;flex-direction:column;gap:12px}
    .breakdown-topline{display:flex;justify-content:space-between;gap:12px;align-items:baseline;margin-bottom:.4rem} .breakdown-label{font-weight:600;min-width:0;word-break:break-word}
    .breakdown-count{color:#bbb;white-space:nowrap;font-size:.92rem} .breakdown-bar-wrap{width:100%;height:12px;background:#17161d;border:1px solid #2a2a35;border-radius:999px;overflow:hidden}
    .breakdown-bar{height:100%;width:0%;background:linear-gradient(90deg,#0ab7f0 0%,#52d3ff 100%);border-radius:999px}
    .breakdown-sub{margin-top:.35rem;color:#bbb;font-size:.9rem;line-height:1.45} .controls{display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;margin:.25rem 0 1rem 0}
...
    .recent-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:16px} .summary-card{background:#1f1e29;border:1px solid #2e2d3b;border-radius:16px;padding:14px}
    .summary-label{color:#bbb;font-size:.88rem;margin-bottom:.25rem} .summary-value{font-size:1.2rem;font-weight:700;word-break:break-word} .summary-sub{margin-top:.35rem;color:#bbb;font-size:.9rem;line-height:1.4}
...
    .video-thumb-link{display:block;width:100%} .video-thumb{display:block;width:100%;aspect-ratio:16 / 9;object-fit:cover;border-radius:12px;background:#111}
    .video-title{display:inline-block;font-weight:700;line-height:1.35;text-decoration:none;word-break:break-word} .video-page-title{margin-top:.45rem;color:#fff;font-size:.95rem;line-height:1.35;word-break:break-word}
    .video-id{color:#bbb;font-size:.9rem;margin-top:.4rem;word-break:break-all} .video-views{margin-top:.5rem;font-size:.95rem;color:#fff} .video-rank{margin-top:.45rem;color:#bbb;font-size:.9rem}
...
    .recent-empty,.error-box{background:#1f1e29;border:1px solid #2e2d3b;border-radius:16px;padding:16px;color:#bbb;line-height:1.5} .api-lines code{white-space:nowrap}
...
    if (!videoId) return
    const arr = memoryStats.recentVideos || []
    const idx = arr.indexOf(videoId)
    if (idx !== -1) arr.splice(idx, 1)
    arr.push(videoId)
    if (arr.length > SETTINGS.telemetry.recentLimit) {
      arr.splice(0, arr.length - SETTINGS.telemetry.recentLimit)
    }
    memoryStats.recentVideos = arr
  }
...
  setInterval(() => {
    if (needsSave && firstUnsavedAt && Date.now() - firstUnsavedAt >= SETTINGS.telemetry.save.maxUnsavedMs) scheduleTelemetrySave(true)
    if (trendingNeedsSave && trendingFirstUnsavedAt && Date.now() - trendingFirstUnsavedAt >= SETTINGS.trending.save.maxUnsavedMs) scheduleTrendingSave(true)
...
    const rawPageTitle = typeof body.pageTitle === "string" ? body.pageTitle : typeof body.title === "string" ? body.title : ""
    const pageTitle = cleanPageTitle(rawPageTitle)
    const categoryId = typeof body.categoryId === "string" ? body.categoryId.trim() : typeof body.videoCategoryId === "string" ? body.videoCategoryId.trim() : ""
    
    const suppliedCategory = typeof body.category === "string"
      ? body.category
      : typeof body.genre === "string"
        ? body.genre
        : typeof body.videoGenre === "string" ? body.videoGenre : ""
        
    const genreTags = collectGenreValues(body.genreTags, body.videoGenreTags, body.tags, body.videoTags, body.tag)
    
...
  <style>
    @font-face { font-family: "PokeTube Flex"; src: url("/static/robotoflex.ttf"); font-style: normal; font-stretch: 1% 800%; font-display: swap; }
    :root { color-scheme: dark; } body { color: #fff; background: #1c1b22; margin: 0; } :visited { color: #00c0ff; } a { color: #0ab7f0; }
    .app { max-width: 1000px; margin: 0 auto; padding: 24px; }
    p{font-family: system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;line-height: 1.6;}
    ul{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:500;font-stretch:extra-expanded;padding-left:1.2rem;}
    h2{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:700;font-stretch:extra-expanded;margin-top:1.5rem;margin-bottom:.3rem;}
    h1{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:1000;font-stretch:ultra-expanded;margin-top:0;margin-bottom:.3rem;}
    .note{color:#bbb;font-size:.95rem;}
    .btn{display:inline-block;margin-top:1rem;padding:.5rem 1rem;border-radius:999px;border:1px solid #2a2a35;background:#252432;color:#fff;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;text-decoration:none;font-size:.95rem;}
    .btn:hover{background:#2f2e3d;} .status{margin-top:.5rem;font-size:.95rem;}
  </style>
</head>
<body>
  <div class="app">
    <h1>Stats opt-out</h1>
    <p>This page lets you turn off <strong>anonymous stats</strong> for this browser. Poke will remember this choice using <code>localStorage</code> only.</p>
    <p class="note">Anonymous stats help us understand which videos are watched through Poke and which platforms people use, without collecting personal data. You can read the full details here: <a href="/policies/privacy#stats">Privacy Policy</a>.</p>
    <a href="#" id="optout-btn" class="btn">Opt out of anonymous stats</a>
    <div id="status" class="status note"></div>
    <p class="note" style="margin-top:1.5rem;">• To see the stats UI, visit <code><a href="/api/stats?view=gui">/api/stats?view=gui</a></code>.<br>• For raw JSON, use <code><a href="/api/stats?view=json">/api/stats?view=json</a></code>.</p>
  </div>
  <script>
    (function () {
      var KEY = "poke_stats_optout"; var btn = document.getElementById("optout-btn"); var status = document.getElementById("status");
...
          }
        } catch (e) { status.textContent = "Your browser blocked localStorage, so we cannot store your opt-out choice." }
      }
...
          updateStatus()
        } catch (e) { status.textContent = "Could not save opt-out preference because localStorage is unavailable." }
      })
      updateStatus()
    })()
  </script>
...
    @font-face { font-family: "PokeTube Flex"; src: url("/static/robotoflex.ttf"); font-style: normal; font-stretch: 1% 800%; font-display: swap; }
    :root{color-scheme:dark} body{color:#fff;background:#1c1b22;margin:0;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    img{float:right;margin:.3em 0 1em 2em;} :visited{color:#00c0ff} a{color:#0ab7f0} .app{max-width:1000px;margin:0 auto;padding:24px;}
    p{font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;line-height:1.6;}
    ul{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:500;font-stretch:extra-expanded;padding-left:1.2rem;}
    h2{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:700;font-stretch:extra-expanded;margin-top:1.5rem;margin-bottom:.3rem;}
    h1{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:1000;font-stretch:ultra-expanded;margin-top:0;margin-bottom:.3rem;}
    hr{border:0;border-top:1px solid #222;margin:28px 0;} .logo{float:right;margin:.3em 0 1em 2em;max-width:130px;}
...
    .tabs{display:inline-flex;background:#15141a;border-radius:24px;padding:4px;border:1px solid rgba(255,255,255,0.05);flex-wrap:wrap;gap:2px;}
    .tab-btn{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;background:transparent;color:#aaa;border:none;padding:8px 20px;border-radius:20px;cursor:pointer;font-weight:700;font-size:.95rem;transition:all .3s ease;outline:none;display:inline-block;line-height:1.2;text-decoration:none;}
    .tab-btn:hover:not(.active){color:#fff;text-decoration:none;} .tab-btn.active{background:#0ab7f0;color:#1c1b22;box-shadow:0 2px 8px rgba(10,183,240,.3);}
    .note{color:#bbb;font-size:.95rem;} .muted{opacity:.8;font-size:.95rem;}
  </style>
</head>
<body>
  <div class="app">
...
    </div>
    <h2>Private by design</h2>
    <p>At <a href="/">Poke</a>, we do not collect or share any personal information. That's our privacy promise in a nutshell. To improve Poke we use a completely anonymous, local-only way to understand how videos are being watched through Poke.</p>
    <p>Any anonymous stats recorded by this instance come through the <code>/api/stats</code> system. You can read exactly what is measured and what is <em>not</em> in our privacy policy: <a href="/policies/privacy#stats">here</a>.</p>
    <hr>
    <h2>API usage</h2>
    <p class="note">These API views are for anonymous local Poke stats only.<br><br>• Main info page: <code><a href="/api/stats">/api/stats</a></code><br>• GUI view: <code><a href="/api/stats?view=gui">/api/stats?view=gui</a></code><br>• Trending tab: <code><a href="/api/stats?view=gui&amp;tab=trending">/api/stats?view=gui&amp;tab=trending</a></code><br>• JSON view: <code><a href="/api/stats?view=json">/api/stats?view=json</a></code><br>• Trending API: <code><a href="/api/trending">/api/trending</a></code><br>• Trending by genre: <code><a href="/api/trending?category=music">/api/trending?category=music</a></code><br>• JSON default limit: <code><a href="/api/stats?view=json">/api/stats?view=json</a></code> (8 videos)<br>• JSON with custom limit: <code><a href="/api/stats?view=json&amp;limit=3000">/api/stats?view=json&amp;limit=3000</a></code><br>• Opt out for this browser: <code><a href="/api/stats/optout">/api/stats/optout</a></code></p>
  </div>
</body>
</html>`)
  }
...
      return res.json({
        startedAt: null, updatedAt: null, generatedAt: getNowIso(), source: "trending.json",
        maxItems: SETTINGS.trending.itemLimit, windowHours: SETTINGS.trending.windowHours, halfLifeHours: SETTINGS.trending.halfLifeHours,
        limit: 0, category: "all", categories: {}, totalTrendingVideos: 0, videos: []
      })
    }
...
      if (!SETTINGS.telemetry.enabled) {
        return res.json({ startedAt: null, videos: {}, pageTitles: {}, recentVideos: [], browsers: {}, os: {}, totalUsers: 0, estimatedTotalUsers: 0, totalVideoIds: 0, totalDetections: 0, limit: 0 })
      }

      const snapshot = getCachedTelemetrySnapshot(typeof req.query.limit !== "undefined" ? req.query.limit : "8")

      return res.json({
        startedAt: snapshot.startedAt, videos: snapshot.videos, pageTitles: snapshot.pageTitles, recentVideos: snapshot.recentVideos,
        browsers: snapshot.browsers, os: snapshot.os, totalUsers: snapshot.totalUsers, estimatedTotalUsers: snapshot.estimatedTotalUsers,
        totalVideoIds: snapshot.totalVideoIds, totalDetections: snapshot.totalDetections, limit: snapshot.limit
      })
    }
...
    p{font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;line-height:1.6;}
    h1,h2,h3,.tab-btn{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    h2{font-weight:700;font-stretch:extra-expanded;margin-top:0;margin-bottom:.4rem;} h1{font-weight:1000;font-stretch:ultra-expanded;margin-top:0;margin-bottom:.35rem;}
    h3{font-weight:700;font-stretch:extra-expanded;margin:0 0 .75rem 0;font-size:1.02rem;} hr{border:0;border-top:1px solid #222;margin:28px 0;}
    .note{color:#bbb;font-size:.95rem;} .logo{float:right;margin:.3em 0 1em 2em;max-width:130px;}
    .header-container{display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;margin-bottom:24px;gap:16px;}
    .tabs{display:inline-flex;background:#15141a;border-radius:24px;padding:4px;border:1px solid rgba(255,255,255,0.05);flex-wrap:wrap;gap:2px;}
    .tab-btn{background:transparent;color:#aaa;border:none;padding:8px 20px;border-radius:20px;cursor:pointer;font-weight:700;font-size:0.95rem;transition:all 0.3s ease;outline:none;display:inline-block;line-height:1.2;text-decoration:none;}
...
    .telemetry-notice{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap;margin:0 0 18px 0;padding:18px;border:1px solid #2a2a35;border-radius:18px;background:#252432;}
    .telemetry-notice h2{margin:0 0 .4rem 0;font-size:1.05rem;} .telemetry-notice p{margin:.4rem 0 0 0;}
    .telemetry-settings{display:flex;align-items:center;justify-content:flex-end;gap:.75rem;flex-wrap:wrap;margin-left:auto;}
    .telemetry-checkbox{display:inline-flex;align-items:center;gap:.45rem;padding:.48rem .76rem;border-radius:999px;border:1px solid #3a3947;background:#1f1e29;color:#fff;cursor:pointer;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-size:.9rem;user-select:none;}
    .telemetry-checkbox input{accent-color:#0ab7f0;} .telemetry-checkbox:has(input:disabled){opacity:.55;cursor:not-allowed;}
    .telemetry-refresh-btn{padding:.48rem .76rem;border-radius:999px;border:1px solid #3a3947;background:#252432;color:#fff;cursor:pointer;font:inherit;font-size:.9rem;}
    .telemetry-refresh-btn:hover{background:#2f2e3d;} .telemetry-refresh-btn[disabled]{opacity:.55;cursor:not-allowed;}
    .nojs-warning{margin-bottom:18px;padding:18px;background:linear-gradient(180deg,#2b2130 0%,#241d29 100%);border:1px solid #5e3a63;border-radius:18px;color:#ffe3eb;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;line-height:1.6;box-shadow:0 10px 30px rgba(0,0,0,.22);}
    .nojs-warning h2{margin:0 0 .55rem 0;font-size:1.2rem;color:#fff;} .nojs-warning p{margin:.45rem 0 0 0;} .nojs-warning ul{margin:.75rem 0 0 1.2rem;padding:0;color:#ffd6e1;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;} .nojs-warning code{white-space:nowrap;}
    .hero{display:grid;grid-template-columns:1.45fr .95fr;gap:16px;align-items:start;margin-bottom:18px;}
    .hero-main, .hero-side{background:#252432;border:1px solid #2a2a35;border-radius:18px;padding:18px;}
    .hero-main p, .hero-side p{margin:.4rem 0 0 0;} .hero-side{display:flex;flex-direction:column;gap:14px;}
    .mini-stat{display:flex;flex-direction:column;gap:.15rem;} .mini-stat-label{color:#bbb;font-size:.92rem;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .mini-stat-value{font-size:1.55rem;font-weight:700;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;}
    .mini-stat-sub{color:#bbb;font-size:.9rem;line-height:1.45;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .shield-btn{display:inline-flex;align-items:center;justify-content:center;width:2rem;height:2rem;border-radius:999px;border:1px solid #3a3947;background:#1f1e29;color:#fff;cursor:pointer;padding:0;font-size:1rem;font-weight:700;}
    .shield-btn:hover{background:#2a2837;} .segmented{display:flex;gap:10px;flex-wrap:wrap;margin:0 0 18px 0;}
    .seg-btn{background:#252432;color:#fff;border:1px solid #2a2a35;border-radius:999px;padding:.58rem .9rem;cursor:pointer;}
    .seg-btn.active{border-color:#0ab7f0;box-shadow:inset 0 0 0 1px #0ab7f0;background:#1f1e29;} .panel{display:none;} .panel.active{display:block;}
    .section-card{background:#252432;border:1px solid #2a2a35;border-radius:18px;padding:18px;margin-bottom:16px;}
    .section-help{margin-bottom:16px;background:#22212d;border:1px solid #2e2d3b;border-radius:16px;padding:16px;} .section-help p{margin:.35rem 0 0 0;}
    .overview-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;} .breakdown-empty{color:#bbb;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .breakdown-list{display:flex;flex-direction:column;gap:12px;} .breakdown-item{font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .breakdown-topline{display:flex;justify-content:space-between;gap:12px;align-items:baseline;margin-bottom:.4rem;} .breakdown-label{font-weight:600;min-width:0;word-break:break-word;}
    .breakdown-count{color:#bbb;white-space:nowrap;font-size:.92rem;} .breakdown-bar-wrap{width:100%;height:12px;background:#17161d;border:1px solid #2a2a35;border-radius:999px;overflow:hidden;}
    .breakdown-bar{height:100%;width:0%;background:linear-gradient(90deg,#0ab7f0 0%,#52d3ff 100%);border-radius:999px;} .breakdown-sub{margin-top:.35rem;color:#bbb;font-size:.9rem;line-height:1.45;}
    .controls{display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;margin:.25rem 0 1rem 0;} .controls label{font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .controls select{background:#252432;color:#fff;border:1px solid #2a2a35;border-radius:10px;padding:.45rem .7rem;font:inherit;}
    .limit-warning{width:100%;color:#bbb;font-size:.95rem;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;margin-top:-.15rem;}
    .compact-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap;margin-bottom:12px;}
    .compact-actions{display:flex;gap:.6rem;flex-wrap:wrap;align-items:center;}
    .action-btn{display:inline-flex;align-items:center;gap:.45rem;padding:.58rem .9rem;border-radius:999px;border:1px solid #2e2d3b;background:#1f1e29;color:#fff;text-decoration:none;cursor:pointer;}
    .action-btn:hover{background:#2a2837;} .action-btn[disabled]{opacity:.5;cursor:not-allowed;}
    .recent-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:16px;} .summary-card{background:#1f1e29;border:1px solid #2e2d3b;border-radius:16px;padding:14px;}
    .summary-label{color:#bbb;font-size:.88rem;margin-bottom:.25rem;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;} .summary-value{font-size:1.2rem;font-weight:700;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;word-break:break-word;}
    .summary-sub{margin-top:.35rem;color:#bbb;font-size:.9rem;line-height:1.4;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .recent-grid, .video-grid{list-style:none;padding-left:0;margin:0;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;}
    .recent-card, .video-card{display:flex;flex-direction:column;gap:10px;background:#252432;border:1px solid #2a2a35;border-radius:16px;padding:12px;min-width:0;}
    .video-thumb-link{display:block;width:100%;} .video-thumb{display:block;width:100%;aspect-ratio:16 / 9;object-fit:cover;border-radius:12px;background:#111;}
    .video-meta{min-width:0;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .video-title{display:inline-block;font-weight:700;line-height:1.35;text-decoration:none;word-break:break-word;} .video-page-title{margin-top:.45rem;color:#fff;font-size:.95rem;line-height:1.35;word-break:break-word;}
    .video-id{color:#bbb;font-size:.9rem;margin-top:.4rem;word-break:break-all;} .video-views{margin-top:.5rem;font-size:.95rem;color:#fff;} .video-rank{margin-top:.45rem;color:#bbb;font-size:.9rem;}
    .recent-badge{display:inline-flex;align-items:center;gap:.35rem;width:max-content;background:#1f1e29;border:1px solid #2e2d3b;color:#fff;border-radius:999px;padding:.35rem .65rem;font-size:.85rem;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .recent-empty, .error-box{background:#1f1e29;border:1px solid #2e2d3b;border-radius:16px;padding:16px;color:#bbb;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;line-height:1.5;}
    .pagination-wrap{margin-top:1rem;display:flex;flex-direction:column;gap:.75rem;} .pagination-info{color:#bbb;font-size:.95rem;font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;}
    .pagination-controls{display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;} .page-btn{background:#252432;color:#fff;border:1px solid #2a2a35;border-radius:10px;padding:.5rem .8rem;cursor:pointer;}
    .page-btn[disabled]{opacity:.5;cursor:not-allowed;} .page-number{min-width:2.2rem;text-align:center;background:#1f1e29;} .page-number.active{border-color:#0ab7f0;box-shadow:inset 0 0 0 1px #0ab7f0;}
    .api-lines code{white-space:nowrap;} .modal-backdrop{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.55);padding:20px;z-index:40;}
    .modal-backdrop.open{display:flex;} .modal{width:min(100%,520px);background:#252432;border:1px solid #2f2e3b;border-radius:20px;padding:18px;box-shadow:0 20px 60px rgba(0,0,0,.45);}
    .modal-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px;} .modal-title{font-family:"PokeTube Flex",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif;font-weight:700;font-stretch:extra-expanded;font-size:1.05rem;}
    .modal-close{width:2rem;height:2rem;border-radius:999px;border:1px solid #3a3947;background:#1f1e29;color:#fff;cursor:pointer;padding:0;} .modal p{margin:.55rem 0 0 0;}
    @media (max-width: 1000px){.recent-grid,.video-grid{grid-template-columns:repeat(3,minmax(0,1fr));}}
    @media (max-width: 900px){.hero{grid-template-columns:1fr;} .overview-grid{grid-template-columns:1fr;} .recent-summary{grid-template-columns:1fr;}}
    @media (max-width: 860px){.recent-grid,.video-grid{grid-template-columns:repeat(2,minmax(0,1fr));}}
    @media (max-width: 640px){.recent-grid,.video-grid{grid-template-columns:1fr;} .breakdown-topline{flex-direction:column;align-items:flex-start;gap:.2rem;}}
...
        <p>This live GUI needs JavaScript for auto-refresh, client-side tabs, pagination, and downloads. A server-rendered snapshot is available and does not auto-refresh.</p>
        <ul>
...
      <div>
        <h2>Telemetry system changed</h2>
        <p class="note">Poke now stores anonymous local telemetry in one <code>telemetry.json</code> file. This GUI reads the JSON API and can refresh itself while you keep this page open.</p>
      </div>
...
      <div class="hero-main">
        <h2>Private by design</h2>
        <p class="note">These stats are aggregated locally on this Poke instance. Video popularity is based on views recorded from Poke's video watch page. For what is collected and what is not, see <a href="/policies/privacy#stats">privacy policy</a>.</p>
        <p class="note" style="margin-top:.7rem;"><strong>Important:</strong> these are local Poke numbers, not public YouTube view counts.</p>
      </div>
      <div class="hero-side">
        <div class="mini-stat">
...
          </div>
          <div class="mini-stat-sub">Private estimate based on anonymous user IDs plus aggregate OS/browser detection patterns. No page-viewer tracking is used here.</div>
        </div>
        <div class="mini-stat">
          <div class="mini-stat-label">total video ids seen</div>
...
      <div class="section-help">
        <h2>How to read recent videos</h2>
        <p class="note">This section shows the most recently recorded video IDs from this Poke instance, in newest-first order. It is useful for checking what this instance has touched lately, not for measuring popularity.</p>
      </div>
      <div class="section-card">
        <div class="compact-head">
          <div>
            <h2>Recently viewed video IDs</h2>
            <p class="note" style="margin:0;">A rolling local list of recent video IDs recorded by this Poke instance. These are not public platform stats.</p>
          </div>
...
      <div class="section-help">
        <h2>Trending videos</h2>
        <p class="note">Trending is calculated from recent local views in <code>trending.json</code>. It uses recency-weighted hourly buckets, keeps at most 400 videos, and auto-categorises genre from the recorded page title.</p>
      </div>
      <div class="section-card">
        <div class="compact-head">
          <div>
            <h2>Trending</h2>
            <p class="note" style="margin:0;">A local, recency-weighted list inspired by trending pages, not public YouTube Trending.</p>
          </div>
        </div>
...
      <div class="section-help">
        <h2>How to read top videos</h2>
        <p class="note">Rankings here are based only on anonymous local detections on this Poke instance. A higher number means this instance saw that video more often, not that the upstream platform reported more public views.</p>
      </div>
      <div class="section-card">
        <div class="compact-head">
          <div>
...
          </select>
          <div id="limit-warning" class="limit-warning" style="display:none;">Warning: this mode may have a lot of pages.</div>
        </div>
...
      </div>
      <p class="note" style="margin:0;">This estimate is based on already-aggregated anonymous stats only. It does not add personal profiles, account linking, or unique page-viewer tracking for this page.</p>
      <p class="note">For more detail about what is measured and what is not, see <a href="/policies/privacy#stats">our privacy policy</a>.</p>
    </div>
  </div>
  <script>
...
    const TAB_TO_PANEL = { overview: "overview-panel", recent: "recent-panel", trending: "trending-panel", topvideos: "top-panel", api: "api-panel" }
    const PANEL_TO_TAB = { "overview-panel": "overview", "recent-panel": "recent", "trending-panel": "trending", "top-panel": "topvideos", "api-panel": "api" }
    function normalizeTabName(value) {
      var raw = String(value || "").trim().toLowerCase().replace(/[\\s_-]+/g, "")
      if (raw === "recent" || raw === "recents") return "recent"
      if (raw === "trend" || raw === "trending" || raw === "trendingvideos") return "trending"
      if (raw === "top" || raw === "topvideo" || raw === "topvideos" || raw === "videos") return "topvideos"
      if (raw === "api" || raw === "json" || raw === "endpoints") return "api"
...
    }
    function getPanelForTab(tab) { return TAB_TO_PANEL[normalizeTabName(tab)] || TAB_TO_PANEL.overview }
...
      panels.forEach(function (panel) { panel.classList.toggle("active", panel.id === cleanPanelId) })
      segButtons.forEach(function (btn) { btn.classList.toggle("active", btn.getAttribute("data-panel") === cleanPanelId) })
      if (updateUrl) updateTabQuery(tab)
    }
    segButtons.forEach(function (btn) { btn.addEventListener("click", function () { setActivePanel(btn.getAttribute("data-panel"), true) }) })
    window.addEventListener("popstate", function () { setActivePanel(getPanelForTab(getTabFromUrl()), false) })
    setActivePanel(getPanelForTab(getTabFromUrl()), false)
    function openPrivacyModal() { privacyModalBackdrop.classList.add("open"); privacyModalBackdrop.setAttribute("aria-hidden", "false") }
    function closePrivacyModal() { privacyModalBackdrop.classList.remove("open"); privacyModalBackdrop.setAttribute("aria-hidden", "true") }
    estimatedUsersInfoBtn.addEventListener("click", openPrivacyModal)
    privacyModalClose.addEventListener("click", closePrivacyModal)
    privacyModalBackdrop.addEventListener("click", function (ev) { if (ev.target === privacyModalBackdrop) closePrivacyModal() })
    document.addEventListener("keydown", function (ev) { if (ev.key === "Escape" && privacyModalBackdrop.classList.contains("open")) closePrivacyModal() })
    function getThumbnailUrl(videoId) { return "https://i.ytimg.com/vi/" + encodeURIComponent(videoId) + "/hqdefault.jpg" }
    function getSelectedLimit() { return parseInt(videoLimitSelect.value, 10) || 100 }
    function getLimitedEntries() { return Object.entries(allVideos).slice(0, getSelectedLimit()) }
    function shouldPaginate() { var selected = getSelectedLimit(); return selected === 1000 || selected === 3000 }
    function updateLimitWarning() { var selected = getSelectedLimit(); limitWarning.style.display = selected === 1000 || selected === 3000 ? "block" : "none" }
    function getTotalPages(entries) { return Math.max(1, Math.ceil(entries.length / CARDS_PER_PAGE)) }
...
      if (disabled) { btn.disabled = true; return btn }
...
      if (!shouldPaginate()) { paginationWrap.style.display = "none"; paginationInfo.textContent = ""; paginationControls.innerHTML = ""; return }
      var totalPages = getTotalPages(entries)
      if (entries.length <= CARDS_PER_PAGE) { paginationWrap.style.display = "none"; paginationInfo.textContent = ""; paginationControls.innerHTML = ""; return }
      paginationWrap.style.display = "flex"; paginationControls.innerHTML = ""
      var startIndex = (currentPage - 1) * CARDS_PER_PAGE + 1
      var endIndex = Math.min(currentPage * CARDS_PER_PAGE, entries.length)
      paginationInfo.textContent = "Showing " + startIndex + "–" + endIndex + " of " + entries.length + " videos. These are local Poke rankings, not YouTube public views."
      paginationControls.appendChild(createPageButton("Prev", currentPage - 1, currentPage === 1, false))
      var startPage = Math.max(1, currentPage - 2)
      var endPage = Math.min(totalPages, currentPage + 2)
      if (startPage > 1) {
        paginationControls.appendChild(createPageButton("1", 1, false, currentPage === 1))
        if (startPage > 2) { var gapLeft = document.createElement("span"); gapLeft.className = "note"; gapLeft.textContent = "…"; paginationControls.appendChild(gapLeft) }
      }
      for (var page = startPage; page <= endPage; page++) { paginationControls.appendChild(createPageButton(String(page), page, false, page === currentPage)) }
      if (endPage < totalPages) {
        if (endPage < totalPages - 1) { var gapRight = document.createElement("span"); gapRight.className = "note"; gapRight.textContent = "…"; paginationControls.appendChild(gapRight) }
        paginationControls.appendChild(createPageButton(String(totalPages), totalPages, false, currentPage === totalPages))
      }
      paginationControls.appendChild(createPageButton("Next", currentPage + 1, currentPage === totalPages, false))
    }
    function formatPercent(part, total) { if (!total) return "0.00"; return ((part / total) * 100).toFixed(2) }
    function sumValues(obj) { return Object.values(obj || {}).reduce(function (sum, value) { return sum + (Number(value) || 0) }, 0) }
...
    }
    function formatLocalViewCount(value) { var count = Number(value) || 0; return count === 1 ? "1 local view" : count + " local views" }
    function getFriendlyOsName(name) {
      if (name === "windows") return "Windows"
      if (name === "android") return "Android"
      if (name === "unknown") return "Unknown"
...
      targetEl.innerHTML = ""
      var entries = Object.entries(data || {}).map(function (entry) { return [entry[0], Number(entry[1]) || 0] }).filter(function (entry) { return entry[1] > 0 }).sort(function (a, b) { return b[1] - a[1] })
      if (entries.length === 0) { var empty = document.createElement("div"); empty.className = "breakdown-empty"; empty.textContent = "No data recorded yet."; targetEl.appendChild(empty); return }
      var total = entries.reduce(function (sum, entry) { return sum + entry[1] }, 0)
      entries.forEach(function (entry) {
        var key = entry[0]; var count = entry[1]; var percent = formatPercent(count, total); var label = kind === "os" ? getFriendlyOsName(key) : getFriendlyBrowserName(key)
        var item = document.createElement("div"); item.className = "breakdown-item"
        var topLine = document.createElement("div"); topLine.className = "breakdown-topline"
        var labelEl = document.createElement("div"); labelEl.className = "breakdown-label"; labelEl.textContent = label + " · " + percent + "%"
        var countEl = document.createElement("div"); countEl.className = "breakdown-count"; countEl.textContent = count + " detections"
        var barWrap = document.createElement("div"); barWrap.className = "breakdown-bar-wrap"
        var bar = document.createElement("div"); bar.className = "breakdown-bar"; bar.style.width = percent + "%"
        var sub = document.createElement("div"); sub.className = "breakdown-sub"; sub.textContent = label + " was detected " + count + " times out of " + total + " total " + (kind === "os" ? "OS detections" : "browser detections") + " on this Poke instance."
        barWrap.appendChild(bar); topLine.appendChild(labelEl); topLine.appendChild(countEl); item.appendChild(topLine); item.appendChild(barWrap); item.appendChild(sub); targetEl.appendChild(item)
      })
    }
    function cleanGuiPageTitle(value) {
      var title = String(value || "").replace(/\\s+/g, " ").trim()
      if (!title) return ""
      title = title.replace(/\\s*(?:\\||-|–|—)\\s*Poke(?:Tube)?\\s*$/i, "").replace(/\\s+/g, " ").trim()
      if (/^Poke(?:Tube)?$/i.test(title)) return ""
      return title.slice(0, 300)
    }
    function getDisplayPageTitle(value) { var title = cleanGuiPageTitle(value); return title || "Couldnt record" }
    function cleanGuiPageTitles(input) {
      var output = {}
      Object.entries(input || {}).forEach(function (entry) {
        var id = String(entry[0] || "").trim(); var title = cleanGuiPageTitle(entry[1])
        if (id && title) output[id] = title
      })
      return output
    }
    function escapeHtml(value) { return String(value || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;") }
    function createVideoCard(videoId, extraText, thumbAlt, href, badgeText, pageTitle) {
      var li = document.createElement("li"); li.className = "video-card"
      var thumbLink = document.createElement("a"); thumbLink.className = "video-thumb-link"; thumbLink.href = href; thumbLink.setAttribute("aria-label", "Open video " + videoId)
      var img = document.createElement("img"); img.className = "video-thumb"; img.src = getThumbnailUrl(videoId); img.alt = thumbAlt; img.loading = "lazy"; img.referrerPolicy = "no-referrer"
      img.onerror = function () { this.style.display = "none" }; thumbLink.appendChild(img)
      var meta = document.createElement("div"); meta.className = "video-meta"
      if (badgeText) { var badge = document.createElement("div"); badge.className = "recent-badge"; badge.textContent = badgeText; meta.appendChild(badge) }
      var titleLink = document.createElement("a"); titleLink.className = "video-title"; titleLink.href = href; titleLink.textContent = videoId
      var pageTitleEl = document.createElement("div"); pageTitleEl.className = "video-page-title"; pageTitleEl.textContent = getDisplayPageTitle(pageTitle)
      var idEl = document.createElement("div"); idEl.className = "video-id"; idEl.textContent = "Video ID: " + videoId
      meta.appendChild(titleLink); meta.appendChild(pageTitleEl); meta.appendChild(idEl); li.appendChild(thumbLink); li.appendChild(meta)
      return li
    }
...
      var payload = {
        exportedAt: new Date().toISOString(), source: "/api/stats?view=gui", note: "These recent video IDs are from anonymous local Poke stats only.",
        totalRecentVideoIds: recentVideoIds.length, recentVideoIds: recentVideoIds.slice(),
...
      var url = URL.createObjectURL(blob); var a = document.createElement("a"); a.href = url; a.download = "poke-recent-video-ids.json"; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url)
    }
    downloadRecentJsonBtn.addEventListener("click", downloadRecentVideoIds)
    function renderRecentVideos() {
      recentVideos.innerHTML = ""; updateRecentSummary()
      if (!Array.isArray(recentVideoIds) || recentVideoIds.length === 0) { recentVideos.innerHTML = '<li class="recent-empty">No recent video IDs recorded yet.</li>'; return }
      recentVideoIds.slice(0, 12).forEach(function (videoId, index) {
        var card = createVideoCard(videoId, "Recently recorded on this Poke instance. Position #" + (index + 1) + " in the live recent queue.", "Thumbnail for recent video " + videoId, "/watch?v=" + encodeURIComponent(videoId), index === 0 ? "Newest ID" : "Position #" + (index + 1), pageTitles[videoId])
        card.className = "recent-card"; recentVideos.appendChild(card)
      })
    }
    function renderTrendingVideos() {
      trendingVideos.innerHTML = ""
      if (!Array.isArray(trendingVideoItems) || trendingVideoItems.length === 0) { trendingVideos.innerHTML = '<li class="error-box">No trending videos recorded yet.</li>'; return }
      trendingVideoItems.slice(0, 100).forEach(function (item, index) {
        var videoId = item.videoId || item.id || ""
        if (!videoId) return
        var li = document.createElement("li"); li.className = "video-card"
        var thumbLink = document.createElement("a"); thumbLink.className = "video-thumb-link"; thumbLink.href = "/watch?v=" + encodeURIComponent(videoId); thumbLink.setAttribute("aria-label", "Open video " + videoId)
        var img = document.createElement("img"); img.className = "video-thumb"; img.src = item.thumbnail || getThumbnailUrl(videoId); img.alt = "Thumbnail for trending video " + videoId; img.loading = "lazy"; img.referrerPolicy = "no-referrer"
        img.onerror = function () { this.style.display = "none" }; thumbLink.appendChild(img)
        var meta = document.createElement("div"); meta.className = "video-meta"
        var badge = document.createElement("div"); badge.className = "recent-badge"; badge.textContent = item.category || "uncategorized"
        var titleLink = document.createElement("a"); titleLink.className = "video-title"; titleLink.href = "/watch?v=" + encodeURIComponent(videoId); titleLink.textContent = videoId
        var rank = document.createElement("div"); rank.className = "video-rank"; rank.textContent = "Trending #" + (index + 1)
        var pageTitleEl = document.createElement("div"); pageTitleEl.className = "video-page-title"; pageTitleEl.textContent = getDisplayPageTitle(item.title)
        var idEl = document.createElement("div"); idEl.className = "video-id"; idEl.textContent = "Video ID: " + videoId
        var viewsEl = document.createElement("div"); viewsEl.className = "video-views"; viewsEl.textContent = formatLocalViewCount(item.recentViews || 0) + " recently"
        meta.appendChild(badge); meta.appendChild(titleLink); meta.appendChild(rank); meta.appendChild(pageTitleEl); meta.appendChild(idEl); meta.appendChild(viewsEl); li.appendChild(thumbLink); li.appendChild(meta); trendingVideos.appendChild(li)
      })
    }
    function renderTopVideos() {
      var entries = getLimitedEntries()
      if (entries.length === 0) { topVideos.innerHTML = '<li class="error-box">No stats recorded yet.</li>'; paginationWrap.style.display = "none"; return }
      var pageEntries = entries; var start = 0
      if (shouldPaginate()) {
        var totalPages = getTotalPages(entries)
        if (currentPage > totalPages) currentPage = totalPages
        if (currentPage < 1) currentPage = 1
        start = (currentPage - 1) * CARDS_PER_PAGE; var end = start + CARDS_PER_PAGE; pageEntries = entries.slice(start, end)
      } else { currentPage = 1 }
      topVideos.innerHTML = ""
      pageEntries.forEach(function (entry, pageIndex) {
        var absoluteIndex = start + pageIndex; var id = entry[0]; var views = entry[1]
        var li = document.createElement("li"); li.className = "video-card"
        var thumbLink = document.createElement("a"); thumbLink.className = "video-thumb-link"; thumbLink.href = "/watch?v=" + encodeURIComponent(id); thumbLink.setAttribute("aria-label", "Open video " + id)
        var img = document.createElement("img"); img.className = "video-thumb"; img.src = getThumbnailUrl(id); img.alt = "Thumbnail for video " + id; img.loading = "lazy"; img.referrerPolicy = "no-referrer"
        img.onerror = function () { this.style.display = "none" }; thumbLink.appendChild(img)
        var meta = document.createElement("div"); meta.className = "video-meta"
        var titleLink = document.createElement("a"); titleLink.className = "video-title"; titleLink.href = "/watch?v=" + encodeURIComponent(id); titleLink.textContent = id
        var rank = document.createElement("div"); rank.className = "video-rank"; rank.textContent = "Rank #" + (absoluteIndex + 1)
        var pageTitleEl = document.createElement("div"); pageTitleEl.className = "video-page-title"; pageTitleEl.textContent = getDisplayPageTitle(pageTitles[id])
        var idEl = document.createElement("div"); idEl.className = "video-id"; idEl.textContent = "Video ID: " + id
        var viewsEl = document.createElement("div"); viewsEl.className = "video-views"; viewsEl.textContent = formatLocalViewCount(views)
        meta.appendChild(titleLink); meta.appendChild(rank); meta.appendChild(pageTitleEl); meta.appendChild(idEl); meta.appendChild(viewsEl); li.appendChild(thumbLink); li.appendChild(meta); topVideos.appendChild(li)
      })
      renderPagination(entries)
    }
    function setDisabledState(message) {
      topVideos.innerHTML = '<li class="error-box">' + message + "</li>"; trendingVideos.innerHTML = '<li class="error-box">' + message + "</li>"; recentVideos.innerHTML = '<li class="error-box">' + message + "</li>"
      videoLimitSelect.disabled = true; downloadRecentJsonBtn.disabled = true; refreshNowBtn.disabled = true; autoRefreshToggle.disabled = true; paginationWrap.style.display = "none"
      osBreakdown.innerHTML = '<div class="breakdown-empty">' + message + "</div>"; browserBreakdown.innerHTML = '<div class="breakdown-empty">' + message + "</div>"
      recentCount.textContent = "0"; recentLatest.textContent = "None"; telemetryStartedAt.textContent = "Not started"
    }
...
      return JSON.stringify({ startedAt: data ? data.startedAt || null : null, videos: videos, recentVideos: recent, browsers: browsers, os: os, pageTitles: titles, totalUsers: data ? data.totalUsers || 0 : 0, estimatedTotalUsers: data ? data.estimatedTotalUsers || 0 : 0, totalVideoIds: data ? data.totalVideoIds || 0 : 0, totalDetections: data ? data.totalDetections || 0 : 0 })
    }
    function applyTelemetryData(data) {
      var videos = data.videos || {}; var titles = cleanGuiPageTitles(data.pageTitles || {}); var recent = data.recentVideos || []; var browsers = data.browsers || {}; var os = data.os || {}
      var totalUsers = data.totalUsers || 0; var estimatedUsers = data.estimatedTotalUsers || 0; var totalVideoIds = data.totalVideoIds || 0; var startedAt = data.startedAt || null
      var signature = getTelemetrySignature(data); var changed = signature !== lastTelemetrySignature
      allVideos = videos; pageTitles = titles; recentVideoIds = recent; lastTelemetrySignature = signature
      userIdCount.textContent = String(totalUsers); estimatedTotalUsers.textContent = String(estimatedUsers); totalVideoIdCount.textContent = String(totalVideoIds); telemetryStartedAt.textContent = formatStartedAt(startedAt)
      if (changed || !hasLoadedTelemetryOnce) { renderBreakdown(osBreakdown, os, "os"); renderBreakdown(browserBreakdown, browsers, "browser"); renderRecentVideos(); updateLimitWarning(); renderTopVideos() }
      hasLoadedTelemetryOnce = true
    }
    function getTrendingSignature(data) { return JSON.stringify({ updatedAt: data ? data.updatedAt || null : null, category: data ? data.category || "all" : "all", videos: data && Array.isArray(data.videos) ? data.videos : [] }) }
    function applyTrendingData(data) {
      var signature = getTrendingSignature(data); var changed = signature !== lastTrendingSignature
      trendingVideoItems = Array.isArray(data.videos) ? data.videos : []; lastTrendingSignature = signature
      if (changed || !hasLoadedTelemetryOnce) renderTrendingVideos()
    }
    function getTrendingJsonUrl() { return "/api/trending?limit=100&_=" + encodeURIComponent(String(Date.now())) }
    function loadTrendingData(force) {
      if (trendingFetchInProgress && !force) return
      trendingFetchInProgress = true
      fetch(getTrendingJsonUrl(), { cache: "no-store", headers: { "Accept": "application/json" } })
        .then(function (res) { if (!res.ok) throw new Error("Trending request failed with status " + res.status); return res.json() })
        .then(function (data) { applyTrendingData(data) })
        .catch(function () { if (!lastTrendingSignature) trendingVideos.innerHTML = '<li class="error-box">Error loading trending data.</li>' })
        .finally(function () { trendingFetchInProgress = false })
    }
    function loadAllData(force) { loadTelemetryData(force); loadTrendingData(force) }
    function getTelemetryJsonUrl() { return "/api/stats?view=json&limit=3000&_=" + encodeURIComponent(String(Date.now())) }
    function loadTelemetryData(force) {
      if (telemetryFetchInProgress && !force) return
      telemetryFetchInProgress = true; refreshNowBtn.disabled = true
      fetch(getTelemetryJsonUrl(), { cache: "no-store", headers: { "Accept": "application/json" } })
        .then(function (res) { if (!res.ok) throw new Error("Telemetry request failed with status " + res.status); return res.json() })
        .then(function (data) { applyTelemetryData(data) })
        .catch(function () { if (!hasLoadedTelemetryOnce) { setDisabledState("Error loading data."); userIdCount.textContent = "Error"; estimatedTotalUsers.textContent = "Error"; totalVideoIdCount.textContent = "Error"; telemetryStartedAt.textContent = "Error" } })
        .finally(function () { telemetryFetchInProgress = false; refreshNowBtn.disabled = false })
    }
    function getAutoRefreshEnabled() { try { return localStorage.getItem(AUTO_REFRESH_KEY) !== "0" } catch (e) { return true } }
    function saveAutoRefreshEnabled(enabled) { try { localStorage.setItem(AUTO_REFRESH_KEY, enabled ? "1" : "0") } catch (e) {} }
    function stopTelemetryAutoRefresh() { if (telemetryRefreshTimer) { clearInterval(telemetryRefreshTimer); telemetryRefreshTimer = null } }
    function startTelemetryAutoRefresh() { stopTelemetryAutoRefresh(); telemetryRefreshTimer = setInterval(function () { loadAllData(false) }, AUTO_REFRESH_MS) }
    document.addEventListener("visibilitychange", function () { if (!document.hidden && autoRefreshToggle.checked) loadAllData(true) })
    videoLimitSelect.addEventListener("change", function () { updateLimitWarning(); renderTopVideos() })
    refreshNowBtn.addEventListener("click", function () { loadAllData(true) })
    autoRefreshToggle.addEventListener("change", function () { var enabled = autoRefreshToggle.checked; saveAutoRefreshEnabled(enabled); if (enabled) { startTelemetryAutoRefresh(); loadAllData(true) } else { stopTelemetryAutoRefresh() } })
    if (!TELEMETRY_ON) { setDisabledState("No data because telemetry is disabled."); userIdCount.textContent = "0"; estimatedTotalUsers.textContent = "0"; totalVideoIdCount.textContent = "0"; telemetryStartedAt.textContent = "Not started" }
    else {
      var optedOut = false; try { optedOut = localStorage.getItem(OPT_KEY) === "1" } catch (e) {}
      if (optedOut) { setDisabledState("Opt-out active, so no stats loaded."); userIdCount.textContent = "Opt-out active"; estimatedTotalUsers.textContent = "Opt-out active"; totalVideoIdCount.textContent = "Opt-out active"; telemetryStartedAt.textContent = "Opt-out active" }
      else { var autoRefreshEnabled = getAutoRefreshEnabled(); autoRefreshToggle.checked = autoRefreshEnabled; autoRefreshToggle.disabled = false; refreshNowBtn.disabled = false; loadAllData(true); if (autoRefreshEnabled) startTelemetryAutoRefresh() }
    }
  </script>
</body>
</html>`)
