...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (audio && typeof audio.play === "function") {
    ...
    audio.play = function() {
      ...
      // ═══════════════════════════════════════════════════════════════
      // HARD INVARIANT: in visible foreground, NEVER play audio when
      // video lacks decoded data (readyState < HAVE_FUTURE_DATA).
      // This is the primary defense against audio-over-frozen-video.
      // Only exception: seek kick window (video is recovering from seek).
      // ═══════════════════════════════════════════════════════════════
      if (_gateVisible && state.firstPlayCommitted && _gateRS < HAVE_FUTURE_DATA && !inSeekKickWindow) {
        return Promise.resolve();
      }

      // Stall flags say video is buffering — block audio even if readyState
      // hasn't dropped yet (readyState lags behind actual stall by 1-2 frames)
      if (_gateVisible && state.firstPlayCommitted) {
        if (state.videoWaiting || state.videoStallAudioPaused) {
          // Even with stall flags, if video genuinely has data, let audio through
          if (_gateRS >= HAVE_FUTURE_DATA && !isForegroundVideoActuallyBuffering()) {
            // stall flag is stale — video recovered, clear it and allow
            state.videoWaiting = false;
            state.videoStallSince = 0;
          } else if (!inSeekKickWindow) {
            return Promise.resolve();
          }
        }
        if (now() < state.stallAudioResumeHoldUntil && _gateRS < HAVE_FUTURE_DATA && !inSeekKickWindow) {
          return Promise.resolve();
        }
      ...
      // safety valve: if stall flags have been stuck for 4s+, force-clear them
      const _gateStallAge = state.stallAudioPausedSince ? (now() - state.stallAudioPausedSince) : 0;
      if (_gateStallAge > 4000 && _gateRS >= HAVE_FUTURE_DATA) {
  ...
  function _bgKeepaliveTick() {
    ...
    // backoff: visible=400ms, background ramps 300→600→1200→2000ms
    const backoffMs = isVisible ? 400 :
      _bgKeepaliveFailCount < 4 ? 300 :
      _bgKeepaliveFailCount < 8 ? 600 :
  ...
  const MakeSureAudioOrVideoDoesntPauseUnlessUserReallyWantsTo = (() => {
    ...
    function _tick() {
      ...
      // CRITICAL: Only restart audio if video has sufficient data (readyState >= 3).
      // Without this gate, audio restarts while video is still buffering,
      // creating a pause-resume-pause oscillation loop.
      if (aPaused && coupledMode && audio && document.visibilityState === "visible" &&
          _msVRS >= HAVE_FUTURE_DATA &&
          !state.videoWaiting && !state.videoStallAudioPaused &&
  ...
  const MakeVideoNotFreezeAfterPlaybackAfterAltTabHapenns = (() => {
    ...
    const FROZEN_FRAME_THRESHOLD = 10; // rAF ticks (~166ms @60Hz) before declaring stall
    const WATCHDOG_MS = 200;      // fallback timer interval
    const MICRO_SEEK_OFFSET = 0.01; // large enough for demuxer to treat as real seek
    const STALL_KICK_COOLDOWN_MS = 2000; // don't re-kick faster than this
    let _lastKickAt = 0;
      ...
      if (!vPaused && vRS >= HAVE_FUTURE_DATA) {
        ...
        if (!timeAdvanced && !framesAdvanced && _lastVideoTime >= 0) {
          ...
          if (_frozenFrames >= FROZEN_FRAME_THRESHOLD && (now() - _lastKickAt) > STALL_KICK_COOLDOWN_MS) {
            ...
            // Micro-seek to flush the GPU surface, then re-play. The _isMicroSeek
            // flag tells seeking/seeked handlers to ignore this.
            state._isMicroSeek = true;
            try { vNode.currentTime = vt + 0.01; } catch {}
            setTimeout(() => { state._isMicroSeek = false; }, 200);
            DONTMAKEITDOUBLEPLAY.resetAll();
            if (getVideoPaused()) {
        ...
        _frozenFrames = 0;
        if (!vPaused && vRS < HAVE_FUTURE_DATA) {
          // Video is "playing" but starved — "waiting" event handler manages audio
        }
      }
    ...
    function onTabReturn() {
      ...
      // Resync to pre-hide position if drifted (background playback can advance position)
      if (_preHideTime >= 0 && isFinite(currentVt) && Math.abs(currentVt - _preHideTime) > SYNC_PRECISION && Math.abs(currentVt - _preHideTime) < 30) {
      ...
      const seekTarget = isFinite(targetTime) && targetTime > 0 ? targetTime : currentVt;
      try { vNode.currentTime = seekTarget + 0.01; } catch {}
...
const HAVE_ENOUGH_DATA = 4;
  ...
  const SmoothTabWelcomeBackManagement = {
    ...
    onTabReturn() {
      ...
      // if media is already playing fine, this is a spurious blur/focus cycle
      ...
      if (videoPlaying && audioPlaying && document.visibilityState === "visible" && playbackHealthy && !shouldAggressiveKick) {
        state.rapidPlayPauseCount = 0;
        state.rapidPlayPauseResetAt = now();
        state.altTabTransitionActive = false;
        state.altTabTransitionUntil = 0;
  ...
  function videoReadyForAudioResume(t = NaN) {
    ...
    if (visibleForeground) {
      ...
      if (!canPlaySmoothAt(vNode, target, 0.12) && bufferedAhead(vNode, target) < 0.12) return false;
      return true;
    }
    ...
    if (coupledMode && audio) {
      ...
      if (!audio.paused && audio.volume > 0.015) {
        ...
        fadeAndPauseAudio(pauseFadeMs, () => {
          ...
          setTimeout(() => { state.isProgrammaticAudioPause = false; }, immediateUserPause ? USER_PROGRAMMATIC_FLAG_CLEAR_MS : 200);
        });
      } else {
        cancelActiveFade();
        try { audio.pause(); } catch {}
        setTimeout(() => { state.isProgrammaticAudioPause = false; }, immediateUserPause ? USER_PROGRAMMATIC_FLAG_CLEAR_MS : 150);
      }
    ...
    try {
      ...
      if (coupledMode && audio && audio.paused) {
        ...
        if (requireVisibleVideoHealth) {
          ...
          // Direct visible user play uses the same video-first rule so video
          // doesn't look frozen while audio/sync work gets ahead of it.
  ...
  function startSeekBufferWait(forCoupled) {
    ...
    const resume = () => {
      ...
      state.intendedPlaying = true;
      state.bufferHoldIntendedPlaying = true;
      state.seekStabilizeUntil = Math.max(state.seekStabilizeUntil, now() + 2600);
    ...
    try {
      if (state.playRequestedDuringSeek || shouldResumeAfterSeek()) {
        ...
        if (coupledMode && audio) {
          ...
          const _audioKickInFlight = !!state.audioPlayInFlight;
          const _audioReadyish = Number(audio.readyState || 0) >= HAVE_CURRENT_DATA;
          const _alreadyAligned = isFinite(_seekVt) && !audio.paused && Math.abs(_atNow - _seekVt) <= 0.35;
          const _keepRunning = _recentSeekKick && (_alreadyAligned || _audioKickInFlight);

          // If seeked already restarted audio at the right spot, don't pause it again.
          if (!_keepRunning && !audio.paused) {
            state.isProgrammaticAudioPause = true;
            try { audio.pause(); } catch {}
            setTimeout(() => { state.isProgrammaticAudioPause = false; }, 50);
          }
          if (isFinite(_seekVt)) {
            const _atAfterPauseCheck = Number(audio.currentTime) || 0;
            if (!_keepRunning || Math.abs(_atAfterPauseCheck - _seekVt) > 0.18) {
              state._allowAudioTimeWrite = true;
          ...
          if (_keepRunning) {
            ...
            // Restart audio when needed; keep the kick path forceful and immediate.
      ...
      // Post-seek rescue retries: early and then spaced out.
      // These fire only when audio is still paused, so they don't double-play.
      [120, 300, 650, 1200, 1900, 2800, 4000, 5400].forEach(delay => {
  ...
  // --- rAF buffer monitor: enforces audio silence while video is buffering.
  // Runs every frame. Catches ANY audio play that sneaks through during a stall —
  // even if caused by keepalive, capture guards, or other recovery systems.
  // Also monitors video readyState proactively: if video is "playing" but readyState
  // drops below HAVE_FUTURE_DATA, it means the browser is about to stall.
  ...
  function bufferMonitorTick() {
    ...
    // if video time hasn't moved in 20+ frames (~333ms) and readyState is low, it's buffering.
    // 20 frames avoids false positives from normal decode timing jitter.
    if (state.intendedPlaying && !getVideoPaused() && document.visibilityState !== "hidden") {
      ...
      if (vt > 0 && vt === _bufMonLastVT) {
        ...
        // mark stall if time frozen for 6+ frames (~100ms) AND readyState confirms low buffer.
        // 20 frames (333ms) was too slow — audio played for a third of a second before kill.
        if (_bufMonStallFrames >= 6 && rs < HAVE_FUTURE_DATA &&
            !state.seeking && !state.seekBuffering && !state.restarting &&
            state.firstPlayCommitted) {
    ...
    if (!audio.paused && state.firstPlayCommitted && document.visibilityState === "visible" &&
      ...
      if (_bufVideoStarved || (_bufStallFlagged && _bufVideoStarved)) {
        if (!state.videoStallAudioPaused) {
          pauseAudioForConfirmedVideoStall();
        } else {
          // already flagged but audio somehow restarted — kill it again
          try { audio.volume = 0; } catch {}
          try { audio.pause(); } catch {}
        }
      }
  ...
  function setupHeartbeat() {
    ...
    const beat = () => {
      ...
      if (coupledMode && audio && !audio.paused && document.visibilityState !== "hidden" && state.videoWaiting) {
        ...
        if (_hbRS < HAVE_FUTURE_DATA &&
            (shouldPauseAudioImmediatelyForForegroundVideoBuffer() || isConfirmedForegroundVideoStall(450))) {
  ...
  function setupMediaErrorHandlers() {
    ...
    const onVideoStalled = () => {
      ...
      if (state.firstPlayCommitted) {
        state.videoWaiting = true;
        state.videoStallSince = state.videoStallSince || now();
        const _stallHoldMs = Math.max(MIN_STALL_AUDIO_RESUME_MS, 300);
        if (shouldForcePauseAudioForForegroundVideoBufferEvent()) {
          armForegroundBufferAudioHold(_stallHoldMs);
          if (!audio.paused) {
            pauseAudioForConfirmedVideoStall(_stallHoldMs);
          }
        } else if (shouldPauseAudioImmediatelyForForegroundVideoBuffer() && !state.videoStallAudioPaused) {
          pauseAudioForConfirmedVideoStall(_stallHoldMs);
        }
      }
      scheduleSync(200);
  ...
  function bindCommonMediaEvents() {
    ...
    video.on("waiting", () => {
      ...
      const _visibleForegroundBufferEvent = shouldForcePauseAudioForForegroundVideoBufferEvent();
      const _waitHoldMs = Math.max(MIN_STALL_AUDIO_RESUME_MS, _waitRS < HAVE_CURRENT_DATA ? 480 : 360);
      if (_visibleForegroundBufferEvent) {
        armForegroundBufferAudioHold(_waitHoldMs);
        if (state._stallAudioPauseTimer) {
          clearTimeout(state._stallAudioPauseTimer);
          state._stallAudioPauseTimer = null;
        }
        if (!audio.paused) {
          pauseAudioForConfirmedVideoStall(_waitHoldMs);
        }
      }

      // Kill audio IMMEDIATELY when video stalls — no delays.
      // The old 150ms delay for borderline readyState let audio play over frozen video.
      // The "playing" event handler resumes audio once video recovers.
    ...
    video.on("playing", () => {
      ...
      if (!_stallRecoveryStable) {
        ...
        state.videoStallSince = state.videoStallSince || now();
        if (coupledMode && audio && !audio.paused) {
          pauseAudioForConfirmedVideoStall(Math.max(MIN_STALL_AUDIO_RESUME_MS, 450));
        }
        armResumeAfterBuffer(8000);
        scheduleSync(0);
        return;
      ...
      // replay/jump-to-start during normal playback.
      const _allowStartupZeroCorrection =
        !state.firstPlayCommitted &&
        !state.startupKickDone &&
        !state.pageFullyLoaded &&
      ...
      if (state.videoStallAudioPaused && state.intendedPlaying && coupledMode && audio) {
        ...
        // Minimal grace so buffer monitor doesn't re-kill within one frame.
        // Keep this very short — if video re-stalls, audio must stop fast.
        state.audioStartGraceUntil = Math.max(state.audioStartGraceUntil, now() + 50);
        execProgrammaticAudioPlay({ squelchMs: 250, force: true, minGapMs: 0 }).catch(() => {});
        state.audioEverStarted = true;
      }
      if (state.seeking || state.seekBuffering) {
      ...
      state.bufferHoldIntendedPlaying = true;
        ...
        // video is playing, start audio (unless NMPBFN is already handling it)
        if (coupledMode && audio && audio.paused && state.intendedPlaying &&
          !userPauseLockActive() && !mediaSessionForcedPauseActive() &&
    ...
    audio.addEventListener("seeking", () => {
      ...
      audio.addEventListener("seeked", () => {
        ...
        video.on("seeking", () => {
          ...
          // Save pre-seek audio volume. Only zero volume for unbuffered seeks
          // (handled below after we check audio.buffered). For buffered seeks,
          // zeroing then restoring causes an audible dropout.
          if (coupledMode && audio && !audio.paused) {
          ...
          if (coupledMode && audio) {
            ...
            if (_seekTargetBuffered) {
              // Fast path: target buffered → just move audio position, keep volume
              ...
              // Buffered seek: no volume dip needed, clear the saved pre-volume
              ...
              // Slow path: target unbuffered → zero volume to mask old buffer playback
              try { audio.volume = 0; } catch {}
              if (!audio.paused) {
                squelchAudioEvents(600);
                try {
        ...
        video.on("seeked", () => {
          ...
          if (coupledMode && audio && isFinite(newTime) && newTime >= 0) {
            ...
            // The old approach fired 15 setTimeout kicks that raced with the
            // buffer monitor and sync loop, causing visible play-pause-play
            // oscillation ("shaking"). Now we do one clean kick per element
            // and let finalizeSeekSync handle any stragglers.
            if (shouldResumeAfterSeek() && !state.endedNaturally) {
              ...
              state.seekKickAudioAllowedUntil = _kickArmedAt + 2000;
              state.seekAudioMustStartUntil = _kickArmedAt + 2200;
              state.isProgrammaticAudioPause = false;
              state.audioEventsSquelchedUntil = 0;
              state.audioPauseUntil = 0;
              state.videoStallAudioPaused = false;
              state.stallAudioResumeHoldUntil = 0;
              state.audioStartGraceUntil = Math.max(state.audioStartGraceUntil, _kickArmedAt + 400);
