...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ensure attributes for mobile compatibility
  try {
  ...
  let lastAT = 0;
  let driftCount = 0;
  let bufferGraceActive = false;

  const STARTUP_GRACE_MS = 2500;
  const MUTE_SQUELCH_MS = 600;
  const AUDIO_EVENT_SQUELCH_MS = 500;
  const SYNC_INTERVAL_MS = 100; // Increased frequency for ultra-tight sync
  
  // Drift Thresholds
  const EPS = 0.1;
  const MICRO_DRIFT = 0.025;   // 25ms - virtually unnoticeable
  const BIG_DRIFT = 0.15;     // 150ms - threshold for rate correction
  const HARD_RESYNC_LIMIT = 0.8; // 800ms - threshold for seek-based resync

  // Remove native loops to prevent desync on wrap-around
  try { 
    videoEl.loop = false; videoEl.removeAttribute('loop'); 
  ...
  function rampVolumeTo(target, ms = 50) {
    target = clamp01(target);
    const from = clamp01(audio.volume);
    if (!isFinite(from) || ms <= 0 || Math.abs(target - from) < 0.001) {
  ...
  async function softMuteAudio(ms = 30) { await rampVolumeTo(0, ms); }
  async function softUnmuteAudio(ms = 50) { 
    if (userMutedVideo || userMutedAudio) return;
  ...
  function safeSetCT(media, t) { 
    try { 
      if (isFinite(t) && t >= 0 && Math.abs(media.currentTime - t) > 0.01) {
        media.currentTime = t; 
      }
    } catch (e) {} 
  }
  ...
  function clearSyncLoop() {
    ...
    try { audio.playbackRate = video.playbackRate(); } catch (e) {}
  ...
  // Sophisticated Speed Sync: Uses proportional-integral logic to match phases
  function applySophisticatedSync(vt, at) {
    if (!intendedPlaying || seekingActive || restarting) return;
    
    const delta = vt - at;
    const baseRate = video.playbackRate() || 1;

    if (Math.abs(delta) > HARD_RESYNC_LIMIT) {
      // Too far apart for speed adjustment, snap audio to video
      safeSetCT(audio, vt);
      audio.playbackRate = baseRate;
    } else if (Math.abs(delta) > MICRO_DRIFT) {
      // Proportional correction: adjust speed by 1% per 50ms of drift, capped at 10%
      const correction = delta * 0.5; 
      const adjustedRate = baseRate + correction;
      try {
        audio.playbackRate = Math.max(baseRate * 0.9, Math.min(baseRate * 1.1, adjustedRate));
      } catch (e) {}
    } else {
      try { audio.playbackRate = baseRate; } catch (e) {}
    }
  }
  ...
  function startSyncLoop() {
    ...
    syncInterval = setInterval(() => {
      ...
      // Ensure Master Playback State
      if (intendedPlaying) {
        // If one is stuck but both are ready, kickstart
        if ((video.paused() || audio.paused) && bothPlayableAt(vt) && !restarting && !seekingActive) {
          playTogether();
        }
        
        // Force slave to match master pause/play state
        if (video.paused() && !audio.paused && !restarting) {
          squelchAudioEvents();
          audio.pause();
        } else if (!video.paused() && audio.paused && !restarting && !bufferGraceActive) {
          squelchAudioEvents();
          audio.play().catch(() => {});
        }
      }

      applySophisticatedSync(vt, at);
