...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function shouldBlockNewAudioStart() {
    ...
    if (!allowHiddenBootstrap) {
      ...
      if (getVideoReadyState() < 3) return true;
      try {
  ...
  async function seamlessBgCatchUp(reason = "resume") {
    try {
      ...
      if (isChromiumOnlyBrowser()) {
        setChromiumBgSettling(1100);
        setChromiumAudioStartLock(700);
      }
      ...
      if (!aPausedNow && isFinite(atNow)) {
        ...
        if (!isFinite(vtNow) || Math.abs(target - vtNow) > 0.20 || vPausedNow) {
          safeFastSeek(videoEl, target);
          safeSetVideoTime(target);
        }
      ...
      const needsIntervention = vPausedNow || aPausedNow || driftNow > 0.30 || resumeOnVisible || bgHiddenWasPlaying;
      ...
      if (doFadeIn) {
        try { await rampVolumeTo(0, 28); } catch {}
      }

      markExplicitPlay(15000);
      setResumeWarm(2400);
      setSmoothNoHold(1900);
      setPauseEventGuard(1900);
      ...
      if (doFadeIn) {
        setTimeout(() => {
          ...
          if (!intendedPlaying) return;
          if (userPauseLockActive()) return;
          rampVolumeTo(volTarget, 120).catch(() => {});
        }, 18);
      } else {
        updateAudioGainImmediate();
      }
  ...
  function setupMediaSession() {
    ...
    try {
      ...
      navigator.mediaSession.setActionHandler("play", () => {
        ...
        Promise.allSettled([playPromise, audioBootstrapPromise]).finally(() => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function shouldBlockNewAudioStart() {
    if (!hasExternalAudio || qua === "medium") return false;
    if (!intendedPlaying || userPauseLockActive() || hardPauseLatchActive() || startupCoupledPendingActive() || mediaSessionForcedPauseActive()) return true;
    if (!isChromiumOnlyBrowser()) return false;
  ...
  const hasExternalAudio = !!audio && audio.tagName === "AUDIO" && !!pickAudioSrc();
  let startupBufferPrimed = !hasExternalAudio;
  let startupCoupledPending = false;
  let startupCoupledPendingReason = "";
  let requestStartupPrime = () => {};

  function startupCoupledPendingActive() {
    return hasExternalAudio && qua !== "medium" && startupCoupledPending && !startupBufferPrimed;
  }

  function clearStartupCoupledPending() {
    startupCoupledPending = false;
    startupCoupledPendingReason = "";
  }

  function startupCoupledReadyNow(t = Number(video.currentTime())) {
    if (!hasExternalAudio || qua === "medium") return true;
    const v = getPlayableVideoEl() || videoEl;
    return canPlayAt(v, t) && canStartAudioAt(t);
  }

  function holdCoupledPlayback(reason = "coupled-hold", holdMs = 380) {
    if (!hasExternalAudio || qua === "medium") return false;

    const vt = Number(video.currentTime());
    if (String(reason || "").startsWith("startup-")) {
      startupCoupledPending = true;
      startupCoupledPendingReason = String(reason || "startup-coupled");
    }

    strictBufferHold = true;
    strictBufferHoldReason = reason;
    strictBufferMissCount = Math.max(strictBufferMissCount, 1);
    strictBufferHoldMinUntil = Math.max(strictBufferHoldMinUntil, performance.now() + Math.max(180, Number(holdMs) || 0));

    if (!isVideoPaused()) execProgrammaticVideoPause();
    if (!audio.paused) execProgrammaticAudioPause(Math.max(260, Number(holdMs) || 0));
    if (isFinite(vt)) safeSetCT(audio, vt);

    return true;
  }

  function requestStartupCoupledPlayback(reason = "startup-coupled") {
    if (!hasExternalAudio || qua === "medium") return false;
    if (startupBufferPrimed) return false;

    startupCoupledPending = true;
    startupCoupledPendingReason = String(reason || "startup-coupled");

    try { requestStartupPrime(); } catch {}

    const t = Number(video.currentTime()) || 0;
    if (!startupCoupledReadyNow(t)) {
      holdCoupledPlayback(reason, 420);
      return true;
    }
  ...
  function startSyncLoop() {
    ...
    syncInterval = setInterval(() => {
      ...
      if (intendedPlaying && hasExternalAudio && !startupBufferPrimed) {
        if (requestStartupCoupledPlayback("startup-sync-loop")) return;
      }
      ...
      if (intendedPlaying && !restarting && !seekingActive && !syncing) {
        if (strictBufferHold) {
          ...
          const canKickFirstAudio = !audioEverStarted && canStartAudioAt(vt);
          let audioKickIssued = false;

          if (!mustHoldAudio && performance.now() >= audioPauseInFlightUntil) {
            if (canKickFirstAudio || !startupAudioHoldActive()) {
              audioKickIssued = true;
              execProgrammaticAudioPlay({
                squelchMs: canKickFirstAudio ? 260 : 320,
                minGapMs: canKickFirstAudio ? 0 : 140,
                force: !!canKickFirstAudio
                ...
                if (started) {
                  updateAudioGainImmediate();
                  if (Math.abs(Number(audio.currentTime) - Number(video.currentTime())) > 0.8) safeSetCT(audio, Number(video.currentTime()));
                  return;
                }

                if (!intendedPlaying || restarting || seekingActive || syncing) return;
                holdCoupledPlayback(!startupBufferPrimed ? "startup-coupled-audio" : "coupled-audio-missing", 420);
                armResumeAfterBuffer(8000);
              }).catch(() => {
                if (!intendedPlaying || restarting || seekingActive || syncing) return;
                holdCoupledPlayback(!startupBufferPrimed ? "startup-coupled-audio" : "coupled-audio-missing", 420);
          ...
          if ((mustHoldAudio || (!audioKickIssued && aPaused)) && intendedPlaying && !strictBufferHold) {
            holdCoupledPlayback(!startupBufferPrimed ? "startup-coupled-audio" : "coupled-audio-hold", 420);
            armResumeAfterBuffer(8000);
          }
        } else if (vPaused && !aPaused) {
          if (!userPauseLockActive()) {
  ...
  async function playTogether() {
    ...
    try {
      ...
      if (hasExternalAudio && !startupBufferPrimed) {
        if (requestStartupCoupledPlayback("startup-play-together")) return;
      }
      ...
      if (vOk !== aOk) {
        holdCoupledPlayback(!startupBufferPrimed ? "startup-coupled-result" : "coupled-play-result", 420);
        if (intendedPlaying) armResumeAfterBuffer(8000);
        return;
      }
      ...
      if (!intendedPlaying && !isVideoPaused()) {
        ...
        const canKickFirstAudio = !audioEverStarted && canStartAudioAt(Number(video.currentTime()));
        if (hasExternalAudio && (!startupBufferPrimed || shouldBlockNewAudioStart() || strictBufferHold || startupAudioHoldActive())) {
          holdCoupledPlayback(!startupBufferPrimed ? "startup-coupled-finally" : "coupled-audio-finally", 420);
          if (intendedPlaying) armResumeAfterBuffer(8000);
        } else if (!userPauseLockActive() && !hardPauseLatchActive() && !strictBufferHold && !mediaSessionForcedPauseActive() && !(androidResumeGuardActive() && isVideoPaused()) && performance.now() >= audioPauseInFlightUntil && !shouldBlockNewAudioStart()) {
          if (canKickFirstAudio || !startupAudioHoldActive()) {
            execProgrammaticAudioPlay({
              squelchMs: canKickFirstAudio ? 260 : 320,
              minGapMs: canKickFirstAudio ? 0 : 140,
              ...
              if (!started && intendedPlaying && !isVideoPaused()) {
                holdCoupledPlayback(!startupBufferPrimed ? "startup-coupled-finally" : "coupled-audio-finally", 420);
                armResumeAfterBuffer(8000);
              }
            }).catch(() => {
  ...
  if (qua !== "medium" && hasExternalAudio) {
    ...
    const maybeStart = () => {
      ...
      const looseReady =
        canPlayAt(getPlayableVideoEl() || videoEl, t0) &&
        canStartAudioAt(t0);
      const eagerStartRequested =
        intendedPlaying ||
        userPlayIntentActive() ||
        startupCoupledPendingActive() ||
        startupAutoplayKickInFlight;

      if (!bothStartupBufferedAt(t0)) {
        if (!(looseReady && (primeWait > 250 || eagerStartRequested))) {
          strictBufferHold = true;
          strictBufferHoldReason = startupCoupledPendingActive() ? "startup-coupled-buffer" : "startup-buffer";
          strictBufferHoldMinUntil = Math.max(strictBufferHoldMinUntil, performance.now() + 520);
          queueStartupReadyPoll();
          return;
        }
      ...
      updateAudioGainImmediate();
      scheduleStartupAutoplayKick();
      if (intendedPlaying && !syncing && !seekingActive && (isVideoPaused() || audio.paused)) {
        playTogether().catch(() => {});
      }
      setTimeout(() => { if (!firstPlayCommitted) startupPhase = false; }, 2500);
    };
    ...
    audio.addEventListener("play", () => {
      ...
      updateMediaSessionPlaybackState();
      if (!startupBufferPrimed) {
        if (requestStartupCoupledPlayback("startup-audio-play")) return;
        queueStartupReadyPoll();
        requestStartupPrime();
        return;
      }
    ...
    videoEl.addEventListener("playing", () => {
      ...
      if (!startupBufferPrimed && hasExternalAudio && qua !== "medium") {
        if (requestStartupCoupledPlayback("startup-video-playing")) return;
      }
    ...
    video.on("play", () => {
      ...
      updateMediaSessionPlaybackState();
      if (!startupBufferPrimed) {
        if (requestStartupCoupledPlayback("startup-video-play")) return;
        queueStartupReadyPoll();
        requestStartupPrime();
        return;
      }
    ...
    video.on("playing", () => {
      ...
      if (!startupBufferPrimed && hasExternalAudio && qua !== "medium") {
        if (requestStartupCoupledPlayback("startup-videojs-playing")) return;
      }
          ...
          if (intendedPlaying) {
            ...
            if (shouldUseBgControllerRetry()) {
              ...
              const needCatch =
                ...
                isVideoPaused() ||
                !!audio.paused ||
                (isFinite(vt1) && isFinite(at1) && Math.abs(at1 - vt1) > 0.30);
